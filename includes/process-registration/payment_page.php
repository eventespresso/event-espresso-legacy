...
function event_espresso_pay($att_registration_id=0) {
	...
	do_action('action_hook_espresso_display_thankyou_page_payment_gateway', $payment_data);

	$attendees = $wpdb->get_results("SELECT ea.*, ed.event_name FROM " . EVENTS_ATTENDEE_TABLE . " ea JOIN " . EVENTS_DETAIL_TABLE . " ed ON ed.id=ea.event_id WHERE ea.registration_id ='" . $att_registration_id . "' ORDER BY ID LIMIT 1");
