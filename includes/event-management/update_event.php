        ...
        if ( $_POST['recurrence_id'] > 0 ) {
            ...
            if ( count( $recurrence_arr ) == 0 )
            {
                ...
                if ( $_POST['recurrence_apply_changes_to'] == 2 )
                {
                    ...
                    $UPDATE_SQL = "SELECT id,start_date,event_identifier FROM " . EVENTS_DETAIL_TABLE . " WHERE recurrence_id = %d and event_status = 'A'";
                }
                else
                {
                    ...
                    $UPDATE_SQL = "SELECT id,start_date,event_identifier FROM " . EVENTS_DETAIL_TABLE . " WHERE start_date >='" . $wpdb->escape( $_POST['start_date'] ) . "' AND recurrence_id = %d and event_status = 'A' ";
                }
                ...
                if ( $recurrence_form_modified && $_POST['recurrence_apply_changes_to'] > 1 )
                {
                    ...
                    if ( $_POST['recurrence_apply_changes_to'] == 2 )
                    {
                        ...
                        $UPDATE_SQL = "SELECT id,start_date,event_identifier FROM " . EVENTS_DETAIL_TABLE . " WHERE recurrence_id = %d and event_status = 'A' ORDER BY start_date";
                    }
                    else
                    {
                        //Update this and upcoming events based on recurrence id and start_date >=start_date
                        ...
                        $DEL_SQL = 'DELETE EDT, EAT FROM ' . EVENTS_DETAIL_TABLE . " EDT
                            ...
                            AND recurrence_id = " . $_POST['recurrence_id'];
                        $UPDATE_SQL = "SELECT id,start_date,event_identifier FROM " . EVENTS_DETAIL_TABLE . " WHERE start_date >='" . $wpdb->escape( $_POST['start_date'] ) . "' AND recurrence_id = %d AND event_status = 'A'  ORDER BY start_date";
                    }
                    ...
                    foreach ( $recurrence_dates as $k => $v ) {
                        $result = $wpdb->get_row( $wpdb->prepare( "SELECT ID FROM " . EVENTS_DETAIL_TABLE . " WHERE recurrence_id = %d and start_date = %s and event_status = 'A'", array( $_POST['recurrence_id'], $k ) ) );
...
else
{
    ...
    $sql = array( 'event_name' => $event_name, 'event_desc' => $event_desc, 'display_desc' => $display_desc, 'display_reg_form'=>$display_reg_form, 'event_identifier' => $event_identifier,
        ...
        'reg_limit' => $reg_limit, 'email_id' => $email_id , 'registration_startT' => $registration_startT, 'registration_endT' => $registration_endT);
    ...
    if ( $_REQUEST['start_time'] != '' )
    {
        foreach ( $_REQUEST['start_time'] as $k => $v ) {
            if ( $v != '' )
            {
                $sql_times = "INSERT INTO " . EVENTS_START_END_TABLE . " (event_id, start_time, end_time) VALUES ('" . $event_id . "', '" . $v . "', '" . $_REQUEST['end_time'][$k] . "')";
                //echo "$sql_times <br>";
    ...
    if ( $_REQUEST['event_cost'] != '' )
    {
        foreach ( $_REQUEST['event_cost'] as $k => $v ) {
            if ( $v != '' )
            {
                ...
                $sql_prices = "INSERT INTO " . EVENTS_PRICES_TABLE . " (event_id, event_cost, surcharge, price_type, member_price, member_price_type) VALUES ('" . $event_id . "', '" . $v . "', '" . $_REQUEST['surcharge'][$k] . "', '" . $_REQUEST['price_type'][$k] . "', '" . $member_price . "', '" . $_REQUEST['member_price_type'][$k] . "')";
                //echo "$sql_prices <br>";
    ...
    if ( $_REQUEST['create_post'] == 'Y' )
    {
		$post_type = $_REQUEST['post_type'];
            ...
            if ( strlen( $_REQUEST['post_id'] ) > 0 )
            {
				...
				//update_post_meta( $post_id, 'event_timezone', $event_timezone );
            }
            else
            {
				...
				//add_post_meta( $post_id, 'event_timezone', $event_timezone );				
            }
