...
$csvfile = "../wp-content/uploads/events.csv";
 ...
 function getCSVValues($string, $separator=","){
    ...
    //echo "The  element is in the array";
		...
		if ($skip >= "1"){
			//Add event data
			$events_sql = "INSERT INTO " . EVENTS_DETAIL_TABLE . " (event_name, event_desc, address, city, state, zip, phone, display_desc, event_identifier, ";
			...
			$events_sql .= " VALUES ('$strings[1]', '$strings[2]', '$strings[3]', '$strings[4]', '$strings[5]', '$strings[6]', '$strings[7]', ";
			$events_sql .= " '$strings[8]','$strings[9]', '" . event_date_display($strings[10], 'Y-m-d') . "', '" . event_date_display($strings[11], 'Y-m-d') . "','$strings[14]', ";
			$events_sql .= " '$strings[16]', '$strings[17]', '$strings[18]', '$strings[19]', '$strings[20]','" . event_date_display($strings[10], 'Y-m-d') . "', '" . event_date_display($strings[11], 'Y-m-d') . "')";
			echo $events_sql;
			...
			//Add times data
			$times_sql="INSERT INTO ".EVENTS_START_END_TABLE." (event_id, start_time, end_time) VALUES ('".$last_event_id."', '" . date("h:i A", strtotime($strings[12])) ."', '" . date("h:i A", strtotime($strings[13])) ."')";
			//echo $times_sql;
			$wpdb->query ( $wpdb->prepare( $times_sql ) );
			print $wpdb->print_error();
			
			//Add price data
			$prices_sql="INSERT INTO ".EVENTS_PRICES_TABLE." (event_id, event_cost) VALUES ('".$last_event_id."', '$strings[15]')";
			//echo $prices_sql;
