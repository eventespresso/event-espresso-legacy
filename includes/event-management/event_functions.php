	...
	if ($num_rows > 0) {
		...
		foreach ($event_categories as $category) {
			...
			if (empty($in_event_category))
				$in_event_category = '';
			$html .= '<p id="event-category-' . $category_id . '"><label for="in-event-category-' . $category_id . '" class="selectit"><input value="' . $category_id . '" type="radio" name="event_category[]" id="in-event-category-' . $category_id . '"' . ($in_event_category == $category_id ? ' checked="checked"' : "" ) . '/> ' . $category_name . "</label></p>";
		}
		$top_div = '';
		$bottom_div = '';
		if ($num_rows > 10) {
