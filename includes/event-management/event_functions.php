...
<?php
	if  ($event_id > 0){
		...
		foreach ($times as $time){
			echo '<li>' . __('Start Time','event_espresso') . ' ' .$time_counter++.': <input size="10"  type="text" name="start_time[]" value="'.$time->start_time.'"> - ' . __('End Time','event_espresso') . ': <input size="10"  type="text" name="end_time[]" value="'.$time->end_time.'"><input type="button" value="Remove" onclick="this.parentNode.parentNode.removeChild(this.parentNode);"/></li>';
		}
	}else{
?>
<li>
...
function addTimeInput(divName){
          ...
          newdiv.innerHTML = "<?php _e('Start Time','event_espresso'); ?> " + (counter) + ": <input type='text' size='10' name='start_time[]'> - <?php _e('End Time','event_espresso'); ?>  <input type='text'  size='10' name='end_time[]'><input type='button' value='Remove' onclick='this.parentNode.parentNode.removeChild(this.parentNode);'/>";
...
?>
          ...
          </p><p><input type="button" value="<?php _e('Add A Price','event_espresso'); ?>" onClick="addPriceInput('dynamicPriceInput');"></p>
