...
else
{
		...
		// Add Timezone
		//$event_timezone = $_REQUEST['event_timezone'];
		...
		$registration_end = array_key_exists('registration_end', $recurrence_arr)?$recurrence_arr['registration_end']:$_REQUEST['registration_end'];
                ...
                if (array_key_exists('recurrence_start_date', $recurrence_arr)){
                    //Recurring event
                   $end_date = $recurrence_arr['recurrence_start_date'];
                }
                elseif ($_REQUEST['end_date'] == '' && $_REQUEST['recurrence_end_date'] !=''){
                    //If they leave the Event Start Date empty, the First Event Date in the recurrence module is selected
                    $end_date = $_REQUEST['recurrence_end_date'];
                } elseif (isset($_POST['recurrence']) && $_POST['recurrence'] == 'Y' && $_REQUEST['end_date'] == '') {
                    $end_date = $_REQUEST['recurrence_manual_dates'][count($_REQUEST['recurrence_manual_dates'])-1];
                } else {
		...
		$sql=array('event_name'=>$event_name, 'event_desc'=>$event_desc, 'display_desc'=>$display_desc, 'display_reg_form'=>$display_reg_form, 'event_identifier'=>$event_identifier,
				   ...
				   'early_disc' => $early_disc, 'early_disc_date' => $early_disc_date, 'early_disc_percentage' => $early_disc_percentage, 'alt_email' => $alt_email, 
				    'question_groups' => $question_groups, 'reg_limit'=>$reg_limit, 'additional_limit'=>$additional_limit, 'recurrence_id'=>$recurrence_id, 'email_id' => $email_id, 'registration_startT' => $registration_startT, 'registration_endT' => $registration_endT);
		...
		//Kieran Oshea's Calendar support
		//Todo replace mysql code with wpdb code. Add calendar id field and update calendar function to event editor.
		if ($_REQUEST['add_to_calendar'] == 'Y'){
			
			$linky = get_option('siteurl').'/?page_id=' . $org_options['event_page_id'] . '&regevent_action=register&event_id=' . $last_event_id . '&name_of_event=' . $event_name;
			
			$cal_sql = "INSERT INTO " . WP_CALENDAR_TABLE . " SET event_title='" . mysql_escape_string($event_name)
			 . "', event_desc='" . mysql_escape_string($event_desc) . "', event_begin='" . mysql_escape_string($start_date) 
				 . "', event_recur='S', event_repeats='0', event_end='" . mysql_escape_string($end_date) . "', event_time='" . mysql_escape_string($start_time) . "', event_author=".$current_user->ID.", event_category=".mysql_escape_string($_REQUEST['calendar_category']).", event_link='".mysql_escape_string($linky)."'";
			if (!$wpdb->query($cal_sql)){
				$error = true;
			}
		}
		...
		if ($_REQUEST['start_time'] != ''){
			foreach ($_REQUEST['start_time'] as $k=>$v){
				if($v != '') {
					$sql3="INSERT INTO ".EVENTS_START_END_TABLE." (event_id, start_time, end_time) VALUES ('".$last_event_id."', '".$v."', '".$_REQUEST['end_time'][$k]."')";
					//echo "$sql3 <br>";
					if (!$wpdb->query($sql3)){
						$error = true;
					}
		...
		if ($_REQUEST['event_cost'] != ''){
			foreach ($_REQUEST['event_cost'] as $k=>$v){
				if($v != '') {
					...
					$sql_price="INSERT INTO ".EVENTS_PRICES_TABLE." (event_id, event_cost, surcharge, price_type, member_price, member_price_type) VALUES ('".$last_event_id."', '".$v."', '".$_REQUEST['surcharge'][$k]."', '".$_REQUEST['price_type'][$k]."', '".$member_price."', '".$_REQUEST['member_price_type'][$k]."')";
					//echo "$sql3 <br>";
		...
		if ($_REQUEST['create_post'] == 'Y'){
			...
			if ($post_type =='post'){
				...
				$my_post['post_category'] = array($_REQUEST['cat']);
				...
				add_post_meta( $post_id, 'event_registration_start', time_to_24hr($start_time) );
				add_post_meta( $post_id, 'event_registration_start', time_to_24hr($end_time) );
				...
				add_post_meta( $post_id, 'event_registration_endT', $event_registration_endT );
				//add_post_meta( $post_id, 'event_timezone', $_REQUEST['event_timezone'] );
				
				espresso_save_meta_box( $post_id );
	...
	if ($error != true){?>
        ...
        <a href="<?php echo get_option('siteurl')?>/?page_id=<?php echo $org_options['event_page_id']?>&amp;regevent_action=register&amp;event_id=<?php echo $wpdb->insert_id;?>" target="_blank"><?php echo stripslashes_deep($_REQUEST['event'])?></a>
