...
//skip the first insert because we do not have the start dates
		...
		if (isset($_REQUEST['start_time']) && !empty($_REQUEST['start_time'])) {
			foreach ($_REQUEST['start_time'] as $k => $v) {
				...
				$sql_time = array('event_id' => $last_event_id, 'start_time' => event_date_display($v, 'H:i') , 'end_time' => event_date_display($_REQUEST['end_time'][$k], 'H:i'), 'description' => sanitize_text_field(stripslashes_deep($_REQUEST['time_desc'][$k])), 'reg_limit' => $time_qty );
				$sql_time_data = array('%d', '%s', '%s', '%s', '%d');
