...
function edit_event($event_id = 0) {
	...
	<?php wp_nonce_field('espresso_form_check', 'ee_update_event'); ?>
			...
			postbox('event-status', 'Event Options', '<p class="inputundersmall"><label for"reg-limit">' . __('Attendee Limit: ', 'event_espresso') . ' </label><input id="reg-limit" name="reg_limit"  size="10" type="text" value="' . $reg_limit . '" /><br />' .
							'<span>(' . __('leave blank for unlimited', 'event_espresso') . ')</span></p><br/>' .
	...
	<?php echo '<a href="#" class="button" onclick="prompt(&#39;Event Shortcode:&#39;, \'[SINGLEEVENT single_event_id=&#34;\' + jQuery(\'#event_identifier\').val() + \'&#34;]\'); return false;">' . __('Shortcode') . '</a>' ?> <?php echo '<a href="#" class="button" onclick="prompt(&#39;Short URL:&#39;, \'' . espresso_reg_url($event_id) . '\'); return false;">' . __('Short URL') . '</a>' ?> <?php echo '<a href="#" class="button" onclick="prompt(&#39;Full URL:&#39;, \'' . home_url() . '/?page_id=' . $org_options['event_page_id'] . '&amp;regevent_action=register&amp;event_id=' . $event_id . '\'); return false;">' . __('Full URL') . '</a>' ?> </div>
			...
			<!-- SMW Helpful Hints Secton -->
			<div id="smw-event-helper" class="meta-box-sortables ui-sortable helptext">
				<div style="display: block;" id="event-date-time" class="postbox">
					<div class="handlediv" title="Click to toggle"><br>
					</div>
					<h3 class="hndle"> <span>
						<?php _e('SMW Helpful Hints', 'event_espresso'); ?>
						</span> </h3>
					<div class="inside">
						<p>If your event gets approved, make sure the following fields are filled in order for your event to show up on the public schedule page:
						<ul>
						<li>Registration Start Date / Time (Keep it January 17, 2012)</li>
						<li>Registration End Date / Time</li>
						<li>Event Start Date / Time</li>
						<li>Event End Date / Time</li>
						<li>Venue Name</li>
						</ul>
						You may also refer to our <a href="http://socialmediaweek.org/local-event-partner/" target="_blank"><strong>Event Partner Guidelines</strong></a>, which provides more information on modifying your event and other guidelines for hosting an event with Social Media Week.</p>
					</div>
				</div>
			</div>
			<!-- END SMW Helpful Hints Secton -->
