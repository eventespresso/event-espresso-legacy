	 ...
	 $results = $wpdb->get_results("SELECT * FROM ". EVENTS_DETAIL_TABLE ." WHERE id ='" . $event_id . "'");
		...
		foreach ($results as $result){
				...
				$additional_limit = $result->additional_limit;
                                ...
                                $end_date = array_key_exists('start_date', $recurrence_arr)?$recurrence_arr['start_date']:$result->start_date;
		...
		$sql=array('event_name'=>$event_name, 'event_desc'=>$event_desc, 'display_desc'=>$display_desc, 'event_identifier'=>$event_identifier, 'address'=>$address, 
		'address2'=>$address2, 'city' => $city, 'state' => $state, 'zip' => $zip, 'country' => $country, 'phone'=>$phone, 'virtual_url'=>$virtual_url, 'virtual_phone'=>$virtual_phone,
		'registration_start'=>$registration_start, 'registration_end'=>$registration_end, 'start_date'=>$start_date,'end_date'=>$end_date, 'allow_multiple'=>$allow_multiple, 
		'send_mail'=>$send_mail, 'is_active'=>$is_active,  'conf_mail'=>$conf_mail, 'use_coupon_code'=>$use_coupon_code, 'member_only'=>$member_only,
		'question_groups'=>$question_groups, 'allow_overflow'=>$allow_overflow, 'overflow_event_id'=>$overflow_event_id, 'additional_limit'=>$additional_limit, 
		'reg_limit'=>$reg_limit, 'email_id' => $email_id);
}
		$sql_data = array('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%d','%d','%d','%d');//last 3 items are number place holders
