	...
	function event_espresso_install_system_names(){
		...
		if ($phone == false)
			...
			if ($wpdb->num_rows == 0){
				...
				$wpdb->insert( $wpdb->prefix . "events_qst_group", array('group_name' => 'Address Information', 'group_identifier'=>sanitize_title_with_dashes('address_information-'.time()), 'system_group' => 1), array('%s', '%s', '%d') );
				...
				$address_group_id = $wpdb->insert_id;				
						...
						/**
						 * Address group
						 * Find address, city, state, and zip ids.  At this point, they will be in the system
						 */
