		...
		if ($question->required == "Y") {
			...
			$field_name = (!is_null($question->system_name))?$question->system_name:$question->question_type . '_' . $question->id;
                ...
                if (is_array($answer) && array_key_exists($event_id, $answer)){
                     ...
                     $answer = $answer[$event_id][$field_name];
			...
			case "TEXT" :
				...
				echo '<input type="text" '. $required . ' id="' . $field_name . '-' . $event_id . '"  name="' . $field_name . $multi_name_adjust . '" size="40" value="' . $answer . '" ' . $disabled . ' /></p>';
				break;
			case "TEXTAREA" :
				echo  '<p class="event_form_field">' . $label;
