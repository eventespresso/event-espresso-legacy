...
if (!function_exists('event_form_build')) {
	function event_form_build($question, $answer = "", $event_id = null, $multi_reg = 0) {
		$required = '';
		...
		if ($question->required == "Y") {
			...
			case "TEXT" :
				...
				echo '<input type="text" '. $required . ' id="' . $field_name . '"  name="' . $field_name . $multi_name_adjust . '" size="40" value="' . $answer . '" ' . $disabled . ' /></p>';
				break;
			case "TEXTAREA" :
				echo  '<p class="event_form_field">' . $label;
			...
			case "SINGLE" :
				...
				foreach ( $values as $key => $value ) {
					...
					echo '<li><input title="' . $question->required_text .'" id="SINGLE_' . $question->id . '_' . $key . '" ' . $required .' name="SINGLE_' . $question->id . '"  type="radio" value="' . $value . '" ' . $checked . ' />' . $value . '</li>';
				}
				echo '</ul>';
				break;
			case "MULTIPLE" :
			...
			case "DROPDOWN" :
				...
				$answers = explode ( ",", $answer );
				echo '<p class="event_form_field">' . $label;
				echo '<select ' . $dd_type . ' ' . $required . ' id="DROPDOWN_' . $question->id . '"  />';
