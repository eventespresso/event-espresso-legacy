...
if (!function_exists('event_form_build')) {
	function event_form_build($question, $answer = "", $event_id = null, $multi_reg = 0, $extra = array()) {
		$required = '';
                ...
                $multi_name_adjust = $multi_reg == 1?"[$event_id][$attendee_number]":'';
		...
		if ($question->required == "Y") {
			...
			$field_name = (!is_null($question->system_name))?$question->system_name:$question->question_type . '_' . $question->id;
                ...
                if (is_array($answer) && array_key_exists($event_id, $answer)){
                     ...
                     $answer = $answer[$event_id]['event_attendees'][$attendee_number][$field_name];
			...
			case "TEXT" :
				...
				echo '<input type="text" '. $required . ' id="' . $field_name . '-' . $event_id . '-' . $attendee_number . '"  name="' . $field_name . $multi_name_adjust . '" size="40" value="' . $answer . '" ' . $disabled . ' /></p>';
				break;
			case "TEXTAREA" :
				echo  '<p class="event_form_field">' . $label;
			...
			case "DROPDOWN" :
				...
				$answers = explode ( ",", $answer );
				echo '<p class="event_form_field">' . $label;
				echo '<select ' . $dd_type . ' ' . $required . ' id="DROPDOWN_' . $question->id . '-' . $event_id . '">';
