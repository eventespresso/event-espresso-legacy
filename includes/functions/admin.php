...
function event_espresso_config_page_scripts() {
	...
	switch ( $_REQUEST['page'] ){
		case ( $_REQUEST['page'] == 'discounts' ):
		case ( $_REQUEST['page'] == 'event_categories' ):
		case ( $_REQUEST['page'] == 'admin_reports' ):
		case ( $_REQUEST['page'] == 'form_builder' ):
			wp_enqueue_script( 'tiny_table',  EVENT_ESPRESSO_PLUGINFULLURL.'scripts/tiny-table.js', array('jquery') );//Events core table script
		break;
 ...
 function getCountriesArray($lang="en"){
	...
	switch($lang){
			default: return array(	
									...
									array(197, 'Suecia', 'SE', 'SWE', 2),
									array(198, 'Suiza', 'CH', 'CHE', 1),
									array(199, 'Suriname', 'SR', 'SUR', 1),
									array(200, 'Switzerland', 'SZ', 'SWZ', 1),
...
function event_espresso_custom_email_info(){
  ...
  <p>[registration_id], [fname], [lname], [phone], [event], [event_link], [event_url], [description], [cost], [company], [co_add1], [co_add2], [co_city],[co_state], [co_zip],[contact], [payment_url], [start_date], [start_time], [end_date], [end_time], [location], [location_phone], [google_map_link]</p>','event_espresso'); ?>
  </div>
...
function espresso_db_dropdown($intIdField, $strNameField, $strTableName, $strOrderField, $current_value, $strMethod="desc", $strDDName="NULL") {
global $wpdb;
 ...
 $strQuery = "select $intIdField, $strNameField from $strTableName order by $strOrderField $strMethod";
   ...
   //print_r($data);
  $strDDName = $strDDName != "NULL" ? $strDDName : $strNameField;
   if ($wpdb->num_rows >0) {
   echo '<select name="' . $strDDName . '">';
   echo '<option value="NULL">' . __('Select Value', 'event_espresso') . '</option>';
