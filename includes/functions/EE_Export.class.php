  ...
  private function __construct() {
	...
	function export_attendees() {
	  ...
	  if (!empty($org_options['full_logging']) && $org_options['full_logging'] == 'Y') {
		  espresso_log::singleton()->log(array('file' => __FILE__, 'function' => __FUNCTION__, 'status' => 'Exporting ' . serialize($tables_to_export)));
	  }
		
		if (isset($_REQUEST['event_id'])){
		  //export just the attendees for 1 event
		  
		  global $wpdb;
		  
		  $event_id = $_REQUEST['event_id'];
		  $attendees_sql = 'Select id, event_id, registration_id, fname, lname, email, date from ' . EVENTS_ATTENDEE_TABLE . ' where event_id=' . $event_id;
		  
		  $attendees = $wpdb->get_results($attendees_sql, ARRAY_A);
		  
		  $attendees_meta_sql = "select ea.id, ea.registration_id, ea.attendee_id, ea.question_id, ea.answer, a.event_id 
        from wp_1_events_answer ea,
         wp_1_events_attendee a 
        where a.id = ea.attendee_id
        and a.event_id = $event_id
        and ea.question_id > 3
        and ea.question_id < 17
        order by ea.attendee_id ASC, ea.question_id ASC"; 
		  
		  $attendee_answers = $wpdb->get_results($attendees_meta_sql, ARRAY_A);
	
	    // hash the answers by attendee id
	    
	    $aa_array = array();
	    
	    if ($attendee_answers){
	      foreach ($attendee_answers as $aa){
	        $attendee_id = $aa['attendee_id'];
	        $question_id =  $aa['question_id'];
	        $answer = $aa['answer'];
	        $aa_array[$attendee_id][$question_id] = $answer;
	      }
	    }
	    ...
	    $table_data[] = array('attendee_id', 'event_id', 'registration_id', 'first name', 'last name', 'email', 'registration date', 'address', 'address2', 'city', 'state', 'postal code', 'country', 'phone', 'company', 'title', 'industry', 'gender', 'twitter', 'linkedin');
      ...
      $filename = $this->generate_filename ($_REQUEST['event_id'] . '-attendees' );
      ...
      if (!empty($org_options['full_logging']) && $org_options['full_logging'] == 'Y') {
        espresso_log::singleton()->log(array('file' => __FILE__, 'function' => __FUNCTION__, 'status' => 'Generated Tables to export'));
      }
	...
	private function process_mult_table_export( $tables_to_export = FALSE ) {
		...
		if ( is_array( $tables_to_export ) ) {
			foreach ( $tables_to_export as $table ) {
				...
				if (!empty($org_options['full_logging']) && $org_options['full_logging'] == 'Y') {
		      espresso_log::singleton()->log(array('file' => __FILE__, 'function' => __FUNCTION__, 'status' => 'Got Table ' . $table));
	  }
			}
		}
		return $table_data;
