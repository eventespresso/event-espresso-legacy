            ...
            if ($wpdb->num_rows == 1) {
                if ($result->event_cost > 0.00) {
                    ...
                    if ($early_price_data = early_discount_amount($event_id, $result->event_cost)) {
                        ...
                        $event_cost = '<span class="'.apply_filters('espresso_filter_hook_css_event_price_value','event_price_value').'">' . $org_options['currency_symbol'] . number_format($result->event_cost, 2) . $message . '</span>';
                    }
...
if (!function_exists('event_espresso_price_dropdown')) {
    ...
    function event_espresso_price_dropdown($event_id, $atts) {
		...
		$label = $label == '' ? '<span class="'.apply_filters('espresso_filter_hook_css_section_title','section-title').'">'.__('Choose an Option: ', 'event_espresso').'</span>' : $label;
        ...
        if ($wpdb->num_rows > 1) {
           //Create the label for the drop down
            ...
            foreach ($results as $result) {
                ...
                if ( $result->event_cost != '0.00' ) {
                    $html .= '<span class="'.apply_filters('espresso_filter_hook_css_event_price_label','event_price_label').'">' . __('Price:', 'event_espresso') . '</span> <span class="'.apply_filters('espresso_filter_hook_css_event_price_value','event_price_value').'">' . $org_options['currency_symbol'] . number_format($result->event_cost, 2) . $message . $surcharge . '</span>';
...
//                    $html .= '<span class="free_event">' . __('Free Event', 'event_espresso') . '</span>';
                    $html .= '<span class="'.apply_filters('espresso_filter_hook_css_free_event','free_event').'">' . $result->price_type . '</span>';
...
function espresso_attendee_admin_price_dropdown($event_id, $atts) {
	...
	$html = '';
	$label = isset($label) && $label != '' ? $label : '<span class="'.apply_filters('espresso_filter_hook_css_section_title','section-title').'">'.__('Choose an Option: ', 'event_espresso').'</span>';
