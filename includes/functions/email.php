...
if (!function_exists('event_espresso_email_confirmations')) {
	function event_espresso_email_confirmations($atts){
		...
		$headers .= "From: " . $org_options['organization'] . " <". $alt_email != '' ? $alt_email : $org_options['contact_email'] . ">\r\n";
		$headers .= "Reply-To: " . $org_options['organization'] . "  <" . $alt_email != '' ? $alt_email : $org_options['contact_email'] . ">\r\n";
		...
		foreach ($attendees as $attendee){
			...
			if ($send_admin_email == 'true'){
				...
				$message .= "<p>".__('Email address is','event_espresso')."  ".$attendee_email.".</p>";
				$message .= $address == '' ? '' : "<p>".__('Address:','event_espresso')." " .$address."<br />";
				$message .= $city == '' ? '' : __('City:','event_espresso')." ".$city."<br />";
				$message .= $state == '' ? '' : __('State:','event_espresso')." ".$state."<br />";
				$message .= $zip == '' ? '' : __('Zip:','event_espresso')." ".$zip."</p>";
				$message .= $phone == '' ? '' : "<p>".__('Phone:','event_espresso')." ".$phone."</p>";
				$message .= $email_questions;
				//echo $message;
				wp_mail($alt_email != '' ? $alt_email : $org_options['contact_email'], stripslashes_deep($event_name), stripslashes_deep($message), $headers);
			}

		//Email Confirmation to Attendee
		if ($send_attendee_email == 'true'){
			$payment_link = get_option('siteurl') . "/?page_id=" . $org_options['return_url'] . "&registration_id=" . $registration_id;
			...
			$ReplaceValues = array(
								...
								$event_url,
								$amount_pd,
								$alt_email != '' ? $alt_email : $org_options['contact_email'],
	...
	function event_espresso_send_cancellation_notice($event_id){
		...
		foreach ($events as $event){
			...
			foreach ($attendees as $attendee){
				...
				$email_body .= '<p>'. __('For more information, please email '. $alt_email != '' ? $alt_email : $org_options['contact_email'],'event_espresso') .'</p>';
...
if (!function_exists('event_espresso_send_invoice')) {
	function event_espresso_send_invoice($registration_id, $invoice_subject, $invoice_message ){
		...
		$results = $wpdb->get_results("SELECT a.*, e.event_name, e.event_desc FROM ". EVENTS_ATTENDEE_TABLE ." a
			...
			$event_url = get_option('siteurl') . "/?page_id=" . $org_options['event_page_id']. "&regevent_action=register&event_id=". $event_id;
			$event_link = '<a href="' . $event_url . '">' . $event_name . '</a>';
			 urlencode(get_option('siteurl') . "/?page_id=" . $org_options['return_url'] . "&amp;registration_id=" . $registration_id);
...
$count= 0;
		...
		foreach ($events as $event){
			...
			//$active=$event->is_active;
			//$send_mail= $event->send_mail;
			$email_message = $email_text != ''? $email_text : $email_message= $event->conf_mail;
			//$email_id= $event->email_id;
			$start_date =  event_date_display($event->start_date);
			$end_date =  $event->end_date;
			$event_address = $event->address;
			...
			foreach ($attendees as $attendee){
				...
				$email_subject = $email_subject != '' ? $email_subject : $event_name;
				
				$email_subject = str_replace($SearchValues, $ReplaceValues, $email_subject);
				
				$email_message = str_replace($SearchValues, $ReplaceValues, $email_message);
