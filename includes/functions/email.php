		...
		foreach ($attendees as $attendee){
			...
			if($start == 0){
				...
				*/
				...
				$payment_url = get_option('siteurl') . "/?page_id=" . $org_options['return_url'] . "&amp;registration_id=" . $registration_id . "&amp;id=".$attendee_id;
				$payment_link = '<a href="' . $payment_url . '">' . __('View Your Payment Details') . '</a>';
				$start = 1;
			}
						
			//Build the ticket link
			$ticket_url = get_option('siteurl') . "/?download_ticket&amp;id=" . $attendee_id . "&amp;registration_id=".$registration_id;
			...
			if ($send_admin_email == 'true'){
				...
				if (!$num_people >0)
					$message .= $num_people == 0? '': "<p><strong>".__('Number of Attendees:','event_espresso')."</strong>  ".$num_people."</p>";
				$message .= "<p><strong>".__('Registration ID:','event_espresso')."</strong>  ".$registration_id."</p>";
				$message .= $txn_id ==''?'':"<p><strong>".__('Transaction ID:','event_espresso')."</strong>  ".$txn_id."</p>";
				$message .= "<p><strong>".__('Email address:','event_espresso')."</strong>  ".$attendee_email."</p>";
				$message .= $ticket_type ==''?'':"<p><strong>".__('Ticket Type:','event_espresso')."</strong>  ".$ticket_type."</p>";
				...
				//echo $message;
				wp_mail($alt_email == '' ? $org_options['contact_email']:$alt_email. ',' . $org_options['contact_email'], stripslashes_deep(html_entity_decode($event_name, ENT_QUOTES,"UTF-8")), stripslashes_deep(html_entity_decode($message, ENT_QUOTES,"UTF-8")), $headers);
			}
			...
			if ($send_attendee_email == 'true'){
				...
				if($send_mail == 'Y'){
					wp_mail($attendee_email, stripslashes_deep(html_entity_decode($email_subject, ENT_QUOTES,"UTF-8")), stripslashes_deep(html_entity_decode(wpautop($email_body), ENT_QUOTES, "UTF-8")), $headers);
				}
				...
				if($send_mail == 'N'){
					wp_mail($attendee_email, stripslashes_deep(html_entity_decode($event_name, ENT_QUOTES,"UTF-8")), stripslashes_deep(html_entity_decode(wpautop($default_replaced), ENT_QUOTES, "UTF-8")), $headers);
				}
			}
