...
//echo "<pre>", print_r($_events), "</pre>";
        ...
        for ( $i = 0; $i < $num_rows; $i++ ) {
			...
			$temp_att_id = $att_id;
                    ...
                    $ticket_url = get_option( 'siteurl' ) . "/?download_ticket&amp;id=" . $att_id . "&amp;registration_id=" . $att_registration_id;
			...
			$temp_event_table .= "
				...
				$questions = $wpdb->get_results("SELECT ea.answer, eq.question
								...
								WHERE ea.attendee_id = '".$att_id."' AND system_name IS NULL ORDER BY eq.sequence asc ");
				...
				$q_counter = 0;
                                ...
                                if ($q_num_rows >0){
                                    $email_questions .= '<tr><td colspan = "6">';
                                    foreach ($questions as $question){
                                            $email_questions .= $question->answer != ''?'<p><strong>'. $question->question . ':</strong><br /> ' . str_replace(',', '<br />', $question->answer) . '</p>':'';
                                            $q_counter++;
                                            if ( $q_counter == $q_num_rows)
                                            $email_questions .= $admin_ticket_link .'</td></tr>';
                                    }
                                };
                    ...
                    $email_subject = $org_options['organization'] . __( ' registration confirmation', 'event_espresso' );
                ...
                if ( $send_attendee_email == 'true' )
                {
                    ...
                    $ReplaceValues = array(
                        ...
                        $_event->phone,
                        $google_map_link,
                        $table_open . $table_heading . $temp_event_table .  $table_close
                    );
				...
				if ($email_questions !=''){
					...
					$admin_message .= $email_questions;
                                ...
                                $email_params = array(
                                    ...
                                    'email_subject' => __('Event Registration Notification', 'event_espresso'),
...
if (!function_exists('event_espresso_send_payment_notification')) {
	function event_espresso_send_payment_notification($atts){
		...
		//Build the ticket link
			$ticket_url = home_url() . "/?download_ticket&amp;id=" . $attendee_id . "&amp;registration_id=".$registration_id;
