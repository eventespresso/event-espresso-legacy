...
function espresso_prepare_email_data($attendee_id, $multi_reg, $custom_data='') {
	global $wpdb, $org_options;
	if (!empty($org_options['full_logging']) && $org_options['full_logging'] == 'Y') {
		espresso_log::singleton()->log(array('file' => __FILE__, 'function' => __FUNCTION__, 'status' => 'Attendee ID: ' . $attendee_id));
	}
	$data = new stdClass;
...
if (!function_exists('event_espresso_email_confirmations')) {
  ...
  if (!empty($org_options['full_logging']) && $org_options['full_logging'] == 'Y') {
			espresso_log::singleton()->log(array('file' => __FILE__, 'function' => __FUNCTION__, 'status' => 'multi_reg ' . $multi_reg));
	}
...
if (!function_exists('event_espresso_send_email')) {
	...
	function event_espresso_send_email($params) {
		...
		$headers .= "From: Social Media Week <" . $org_options['contact_email'] . ">\r\n";
