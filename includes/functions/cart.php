...
?>
        ...
        <?php
            ...
            foreach ( $result as $r ) {
                ...
                $meta['additional_attendee_reg_info'] = (is_array( $event_meta ) && array_key_exists( 'additional_attendee_reg_info', $event_meta )) ? $event_meta['additional_attendee_reg_info'] : 2;
...
function event_espresso_copy_dd( $event_id, $meta ) {
    ...
    $var .= '<label>Same as (optional): </label>';
     ...
     * 4) produce the dropdown if it is not the same price id
     */
            ...
            if ( isset( $v_price_id['attendee_quantity'] ) && $v_price_id['attendee_quantity'] > 0 )
            {
                ...
                for ( $i = 1; $i <= $v_price_id['attendee_quantity']; $i++ ) {
                    ...
                    if ( $meta['price_id'] != $k_price_id )
                        $var .= "<option value='$event_id|{$meta['price_id']}|{$meta['attendee_number']}|$k_event_id|$k_price_id|$i'>" . stripslashes_deep( $v_event_id['event_name'] ) . ' - ' . $v_price_id['price_type'] . ' - Attendee ' . $i . "</option>";
                }
            }
        }
    }
...
<?php
    ...
    if ( !function_exists( 'event_espresso_group_price_dropdown' ) )
    {
        ...
        function event_espresso_group_price_dropdown( $event_id, $label = 1, $multi_reg = 0, $value = '' ) {
            ...
            $results = $wpdb->get_results( "SELECT ept.id, ept.event_cost, ept.surcharge, ept.price_type, edt.allow_multiple, edt.additional_limit
...
?>
                <tr>
                    <td colspan="3" style="font-size:9px;">You can register a maximum of <?php echo $attendee_limit; ?> attendees for this event.</td>
