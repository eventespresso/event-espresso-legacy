...
//		printr( $events_in_session, '$events_in_session  <br /><span style="font-size:10px;font-weight:normal;">' . __FILE__ . '<br />line no: ' . __LINE__ . '</span>', 'auto' );
		...
		if ( $event_count > 0 ) {
			...
			$reg_page_url = add_query_arg('regevent_action', 'post_multi_attendee', apply_filters('espresso_filter_permalink', get_permalink($org_options['event_page_id']), $org_options['event_page_id']));
...
<div class = "event_espresso_form_wrapper">
	<form id="event_espresso_checkout_form" method="post" action="<?php echo $reg_page_url ?>">
		<?php
					...
					$cart_page_url = add_query_arg('regevent_action', 'show_shopping_cart', apply_filters('espresso_filter_permalink', get_permalink($org_options['event_page_id']), $org_options['event_page_id']));
...
if (!function_exists('event_espresso_cart_link')) {
	function event_espresso_cart_link($atts) {
		...
		if ( $view_cart || is_array( $events_in_session ) && isset( $events_in_session[ $event_id ] )) {
			$registration_cart_url = add_query_arg('regevent_action', 'show_shopping_cart', apply_filters('espresso_filter_permalink', get_permalink($org_options['event_page_id']), $org_options['event_page_id']));
			...
			$registration_cart_url = isset($externalURL) && $externalURL != '' ? $externalURL : add_query_arg('event_id', $event_id, apply_filters('espresso_filter_permalink', get_permalink($org_options['event_page_id']), $org_options['event_page_id']));
