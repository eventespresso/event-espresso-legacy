	...
	function event_espresso_time_dropdown($event_id = 'NULL', $label = 1, $multi_reg = 0, $value = ''){
		...
		if ($wpdb->num_rows == 1) {//If one result, then display the times.
			...
			foreach ($event_times as $time){
				...
				echo '<input type="hidden" name="start_time_id' . $multi_name_adjust . '" id="start_time_id' . $time->end_time . '" value="' .$time->id .'">';
			}
			...
			foreach ($event_times as $time){
				...
				switch ($time_reg_limit){//This checks to see if the time slots are controlled by registration limits. 
					...
					//Then we need to check if there are enough spaces available.
						...
						echo '<option' . $selected . ' value="' . $time->id . '">' . $time->start_time . ' - ' . $time->end_time . " ($time->available_spaces " . __('available spaces', 'event_espresso') . ")" . '</option>';
					break;
					case 'N'://If time slots are not controlled by registration limits, then we show the default dropdown list of times.
					default:
						echo '<option ' . $selected . ' value="' . $time->id . '">' . $time->start_time . ' - ' . $time->end_time . '</option>';
					break;
				}
			}
			echo '</select>';
...
function eventespresso_ddtimezone($event_id = 0)
{
	...
	$list = DateTimeZone::listAbbreviations();
		...
		echo '<select name="event_timezone" id="event_timezone-">';
			...
			if($event_id>0)
			{
			$tz_event = $wpdb->get_var($wpdb->prepare("SELECT event_timezone FROM " . EVENTS_DETAIL_TABLE . " WHERE id = '" . $event_id . "'"));
