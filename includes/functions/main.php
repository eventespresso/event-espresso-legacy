...
if (!function_exists('event_espresso_additional_attendees')) {
	...
	function event_espresso_additional_attendees($event_id = 0, $additional_limit = 2, $available_spaces = 999, $label = '', $show_label = true, $event_meta = '') {
		...
		if (isset($event_meta['additional_attendee_reg_info']) && $event_meta['additional_attendee_reg_info'] == 1) {
			...
			$htm .= '</div>';
			echo $htm;
			$additional_attendee_form = ob_get_contents();
			ob_end_clean();
			ob_start();
			?>
			<script type="text/javascript">
				...
				var attendee_num = 0;
				var additional_limit = <?php echo $additional_limit; ?>;
				var first_add_button = null;
				var selector = 'div#additional_attendee_' + attendee_num;
				function markup(attendee_num) {
					return '<?php echo stripslashes($htm); ?>';
				}
				function remove_add() {
					attendee_num -= 1;
					selector = 'div#additional_attendee_' + attendee_num;
					$jaer(selector).remove();
					if (attendee_num != 0) {
						var temp_selector = 'div#additional_attendee_' + (attendee_num - 1);
						$jaer(temp_selector + ' a.add').on('click',add_add);
						$jaer(temp_selector + ' a.remove').on('click',remove_add);
					} else {
						first_add_button.on('click',add_add);
					}
				}
				function add_add() {
					if (attendee_num == additional_limit) return;
					$jaer(this).parent().after(markup(attendee_num));
					$jaer(selector + ' a.add').on('click',add_add);
					$jaer(selector + ' a.remove').on('click',remove_add);
					$jaer(this).off('click', add_add);
					if (attendee_num != 0) {
						var temp_selector = 'div#additional_attendee_' + (attendee_num - 1);
						$jaer(temp_selector + ' a.remove').off('click', remove_add);
					}
					attendee_num += 1;
					selector = 'div#additional_attendee_' + attendee_num;
				}
				jQuery(document).ready(function($jaer) {
					$jaer('a.add').on('click',add_add);
					first_add_button = $jaer('a.add');
				});
			</script>
