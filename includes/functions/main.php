...
if (!function_exists('event_espresso_get_is_active')) {
	function event_espresso_get_is_active($event_id){
		...
		elseif ($is_active == "Y" && $event_status == "S"){
			$event_status = array('status'=>'SECONDARY', 'display'=>'<span style="color: #090; font-weight:bold;">'.__('SECONDARY','event_espresso').'</span>', 'display_custom'=>'<span class="espresso_secondary">'.__('Secondary','event_espresso').'</span>');
			//print_r( $event_status);
			return $event_status;
		}
