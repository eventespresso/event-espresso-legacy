...
if (!function_exists('event_espresso_get_is_active')) {
	function event_espresso_get_is_active($event_id){
		...
		//$event_timezone =  $wpdb->last_result[0]->event_timezone;
		//$t = time();
		//$today = date_at_timezone("Y-m-d H:i", $event_timezone, $t);
		...
		//IF the event is a secondary event, show as secondary
		...
		elseif ($is_active == "Y" && date($registration_end_timestamp) <= date(time()) && $event_status != "D"){
			$event_status = array('status'=>'REGISTRATION_CLOSED', 'display'=>'<span style="color: #F00; font-weight:bold;">'.__('CLOSED','event_espresso').'</span>');
			//print_r( $event_status);
			return $event_status;
		}
		...
		elseif ($is_active == "Y" && date($registration_start_timestamp) >= date(time()) && $event_status != "D"){
			$event_status = array('status'=>'REGISTRATION_NOT_OPEN', 'display'=>'<span style="color: #090; font-weight:bold;">'.__('NOT_OPEN','event_espresso').'</span>');
			//print_r( $event_status);
			return $event_status;
		}
		...
		//If the registration start date is less than the current date
		elseif ($is_active == "Y" && date($registration_start_timestamp) <= date(time())  && $event_status != "D"){
			$event_status = array('status'=>'REGISTRATION_OPEN', 'display'=>'<span style="color: #090; font-weight:bold;">'.__('OPEN','event_espresso').'</span>');
			//print_r( $event_status);
			return $event_status;
		}
	...
	function get_number_of_attendees_reg_limit($event_id, $type = 'NULL', $full_text = 'EVENT FULL'){
			...
			switch($type){
				case 'number_available_spaces' :
					...
					$attendees = $wpdb->get_results("SELECT quantity FROM ". EVENTS_ATTENDEE_TABLE . " WHERE event_id='" . $event_id . "' AND quantity > 1 AND payment_status='Completed' ");
				...
				case 'all_attendees' :
					$wpdb->get_results("SELECT id FROM ". EVENTS_ATTENDEE_TABLE . " WHERE payment_status='Completed' ");
					$num_attendees = $wpdb->num_rows;
					
					
					$attendees = $wpdb->get_results("SELECT quantity FROM ". EVENTS_ATTENDEE_TABLE . " WHERE quantity > 1 AND payment_status='Completed' ");
	...
	function event_espresso_price_dropdown($event_id, $label = 1, $multi_reg = 0, $value = '') {
		...
		if ($wpdb->num_rows > 1) {
			...
			foreach ($results as $result){
				...
				echo '<input type="hidden" name="price_id' . $multi_name_adjust . '" id="price_id-' . $event_id . '" value="' . $result->id . '">';
			}
		}else if ($wpdb->num_rows == 0){
...
if (!function_exists('event_espresso_add_question_groups')) {
	function event_espresso_add_question_groups($question_groups, $answer= '', $event_id = null, $multi_reg = 0){
		global $wpdb;
			...
			foreach ($question_groups as $g_id) $questions_in .= $g_id . ',';
				...
				if ($num_rows > 0 ){
					foreach($questions as $question){
						...
						event_form_build($question, $answer, $event_id, $multi_reg);
