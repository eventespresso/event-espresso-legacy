...
if (!function_exists('event_espresso_add_question_groups')) {
	function event_espresso_add_question_groups($question_groups, $answer= '', $event_id = null, $multi_reg = 0, $extra = array()){
		global $wpdb;
		if (count($question_groups) > 0){
			$questions_in = '';

                //Only personal inforamation
                if($extra['additional_attendee_reg_info'] == '2')
			...
			foreach ($question_groups as $g_id) $questions_in .= $g_id . ',';
				...
				if ($num_rows > 0 ){
					foreach($questions as $question){
						...
						event_form_build($question, $answer, $event_id, $multi_reg, $extra);
