	...
	if ($use_coupon_code == "Y"){
		if ($_REQUEST['coupon_code'] != ''){
			...
			if ($wpdb->num_rows > 0) {	
				...
				_e('<p id="event_espresso_invalid_coupon"><font color="red">Sorry, that coupon code is invalid or expired.</font></p>','event_espresso');
			}
		}
	}
	return $event_cost;
...
function event_espresso_coupon_registration_page($use_coupon_code, $event_id){
	if ($use_coupon_code == "Y"){ ?>
		<p class="event_form_field coupon_code" id="coupon_code-<?php echo $event_id;?>">
            <label for="coupon_code"><?php _e('Do you have a promo code?','event_espresso'); ?></label><br />
            <input type="text" tabIndex="9" maxLength="25" size="35" name="coupon_code" id="coupon_code-<?php echo $event_id;?>">
		</p>
<?php
	}
}