...
<li>
          ...
          <div class="box-mid-body" id="toggle6">
            <div class="padding">
              <?php  if($_REQUEST['event_espresso_admin_action'] == 'copy_templates') {
	add_action('admin_init', 'event_espresso_smartCopy');
}?>
              <?php
			  if($_SESSION['event_espresso_themes_copied'] == true) {
					?>
			...
			$files = array('attendee_list.php', 'event_list.php', 'event_list_display.php', 'event_post.php', 'payment_page.php', 'registration_page_display.php', 'registration_page.php', 'return_payment.php', 'widget.php');
			//echo EVENT_ESPRESSO_TEMPLATE_DIR . $files[3];
			if (file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[0]) || file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[1]) || file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[2]) || file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[3]) || file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[4]) || file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[5]) || file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[6]) || file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[7]) || file_exists(EVENT_ESPRESSO_TEMPLATE_DIR . $files[8])){ 
				?>
              <p>
                <?php _e("Theming your event listings and registration pages is easy.");?>
              </p>
              <p>
...
?>
              ...
              <?php
				...
				?>
              <p>
                <?php _e('In order to use this this feature, you will need to move the files located in the <span class="display-path"><strong>' . EVENT_ESPRESSO_PLUGINFULLPATH . 'templates/</strong></span> directory into the <span class="display-path"><strong>' . EVENT_ESPRESSO_TEMPLATE_DIR . '</strong></span> directory.','event_espresso'); ?>
              </p>
			...
			?>
             <p>
        <input class="button-primary" type="submit" name="Submit" value="<?php _e('Save Options', 'event_espresso'); ?>" id="save_organization_saetting_4" />
      </p>
            </div>
          </div>
