...
$deprecated = '<div class="ui-widget">
				<div style="padding: 0 .7em;" class="ui-state-error ui-corner-all">
					<p><span style="float: left; margin-right: .3em;" class="ui-icon ui-icon-alert"></span>
					<strong>DISCONTINUED:</strong>' . __(' Scroll down and select additional attendee questions below.','event_espresso') . '</p>
				</div>
			</div>';

     	if (get_option('event_espresso_multi_reg_active') == 1)
			$additional_attendee_reg_info_dd = '<p>' .__('Additional Attendee Registration info?','event_espresso') . 
                            $deprecated . '</span> ' . select_input('additional_attendee_reg_info', $additional_attendee_reg_info, $event_meta['additional_attendee_reg_info']) .'</p>';
