...
*/
		...
		if ($num_rows > 0) {
			...
			$help_div .= '<p><strong><a href="http://eventespresso.com/wiki/shortcodes-template-variables/?utm_source=ee_plugin_admin&utm_medium=link&utm_content=venue+shortcode+examples+ee_version_'.EVENT_ESPRESSO_VERSION .'+ee_install_url_'.site_url().'&utm_campaign=event_editor_venue_section#venue_shortcode" target="_blank">More Examples</a></strong></p>';
			$help_div .= '</div>';
			ob_start();
	...
	function espresso_personnel_cb($event_id = 0, $recurrence_id = 0){
		...
		if ($num_rows > 0){
			...
			<div id="staff_info" style="display:none">
				...
				<p><strong><a href="http://eventespresso.com/wiki/shortcodes-template-variables/?utm_source=ee_plugin_admin&utm_medium=link&utm_content=Staff+Shortcode+examples<?php echo '+ee_version_'.EVENT_ESPRESSO_VERSION .'+ee_install_url_'.site_url(); ?>&utm_campaign=event_editor_staff_section#staff_shortcode" target="_blank"><?php _e('More Examples', 'event_espresso'); ?></a></strong></p>
			</div>
		</div>
	</div>
<?php
