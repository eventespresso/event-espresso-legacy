...
if (!function_exists('display_event_espresso_categories')) {
	function display_event_espresso_categories($event_category_id="null", $css_class=NULL){
		...
		if ($event_category_id != "null"){
			...
			$sql = "SELECT e.*, c.category_name, c.category_desc, c.display_desc, c.category_identifier FROM ". EVENTS_DETAIL_TABLE . " e ";
...
if (!function_exists('event_espresso_get_event_details')) {
	function event_espresso_get_event_details($sql, $css_class=NULL){
		...
		$category_identifier = $wpdb->last_result[0]->category_identifier;
