	...
	function event_espresso_get_event_details($sql){
		...
		foreach ($events as $event){
			...
			$registration_url = $externalURL != '' ? $externalURL : get_option('siteurl') . '/?page_id='.$event_page_id.'&regevent_action=register&event_id='. $event_id . '&name_of_event=' . stripslashes_deep($event_name);
                        ...
                        if (is_array($events_in_session) && array_key_exists($event_id,$events_in_session)){
                            $registration_cart_url = get_option('siteurl') . '/events/?regevent_action=show_shopping_cart';
                            $registration_cart_anchor = __("View Cart", 'event_espresso');
                        } else {
                            $registration_cart_url = $externalURL != '' ? $externalURL : get_option('siteurl') . '/?page_id='.$event_page_id.'&regevent_action=add_to_session&event_id='. $event_id . '&name_of_event=' . stripslashes_deep($event_name);
                            $registration_cart_anchor = __("Add to Cart and Add More Events", 'event_espresso');
                            $registration_cart_class = 'add_event_to_session';
