...
<?php
		...
		?>
				<div class="<?php echo apply_filters('espresso_filter_hook_css_event_display_boxes','multi_reg_cart_block event-display-boxes ui-widget'); ?>"  id ="multi_reg_cart_block-<?php echo $r->id ?>">
		
					<h3 class="<?php echo apply_filters('espresso_filter_hook_css_event_title','event_title ui-widget-header ui-corner-top'); ?>"><?php echo stripslashes_deep( $r->event_name ) ?> <span class="<?php echo apply_filters('espresso_filter_hook_css_remove_cart_item','remove-cart-item'); ?>"> <img class="<?php echo apply_filters('espresso_filter_hook_css_ee_delete_item_from_cart','ee_delete_item_from_cart'); ?>" id="cart_link_<?php echo $r->id ?>" alt="<?php _e('Remove this item from your cart', 'event_espresso'); ?>" src="<?php echo EVENT_ESPRESSO_PLUGINFULLURL ?>images/icons/remove.gif" /> </span> </h3>
						<div class="<?php echo apply_filters('espresso_filter_hook_css_event_data_display','event-data-display ui-widget-content ui-corner-bottom'); ?>">
							<table id="cart-reg-details" class="<?php echo apply_filters('espresso_filter_hook_css_event_display_tables','event-display-tables ui-widget-content ui-corner-bottom'); ?>">
								<thead>
									<tr>
...
//		printr( $_SESSION, '$_SESSION  <br /><span style="font-size:10px;font-weight:normal;">' . __FILE__ . '<br />line no: ' . __LINE__ . '</span>', 'auto' );
		?>
		<div class="<?php echo apply_filters('espresso_filter_hook_css_event_display_boxes','event-display-boxes ui-widgetevent-display-tables'); ?> ">
			...
			<div id="event_espresso_coupon_wrapper" class="<?php echo apply_filters('espresso_filter_hook_css_coupon_wrapper','clearfix event-data-display'); ?>">
				<label class="<?php echo apply_filters('espresso_filter_hook_css_coupon_code','coupon-code'); ?>" for="event_espresso_coupon_code"><?php _e( 'Enter Coupon Code ', 'event_espresso' ); ?></label>
				<input type="text" 
			...
			<div id="event_espresso_coupon_wrapper" class="<?php echo apply_filters('espresso_filter_hook_css_coupon_wrapper','clearfix event-data-display'); ?>">
				<label class="<?php echo apply_filters('espresso_filter_hook_css_coupon_code','coupon-code'); ?>" for="event_espresso_groupon_code"><?php _e( 'Enter Voucher Code ', 'event_espresso' ); ?></label>
				<input type="text" 
			...
			<div id="event_espresso_total_wrapper" class="<?php echo apply_filters('espresso_filter_hook_css_total_wrapper','clearfix event-data-display'); ?>">					
				<?php do_action( 'action_hook_espresso_shopping_cart_before_total' ); ?>				
				<span class="<?php echo apply_filters('espresso_filter_hook_css_total_price','event_total_price'); ?>">
