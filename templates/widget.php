	...
	class Event_Espresso_Widget extends WP_Widget {
		...
		function widget($args, $instance ) {
			...
			if ( $title )
				...
				if ($instance['category_name'] != 'NULL'){
					$type = 'category';
				}
					...
					//Old Way
					/*$display_recurrence_event = false;//To show recurring events, set to true.
					...
					$sql .= " ORDER BY date(start_date), id LIMIT 5";*/
					...
					//event_espresso_get_event_details($sql);
						foreach ($events as $event){
							...
							?>
							<p><a href="<?php echo get_option('siteurl')?>/?page_id=<?php echo $event_page_id?>&amp;regevent_action=register&amp;event_id=<?php echo $event_id?>&amp;name_of_event=<?php echo stripslashes_deep($event_name)?>"><?php echo stripslashes_deep($event_name)?> - <?php echo event_date_display($start_date)?></a></p>
							<?php
