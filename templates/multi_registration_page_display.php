...
<div id="event_espresso_registration_form">
    ...
    <h2 class="event_title" id="event_title-<?php echo $event_id; ?>"><?php echo "Event $event_counter: " . stripslashes_deep( $event_name ) ?> <?php echo $is_active['status'] == 'EXPIRED' ? ' - <span class="expired_event">Event Expired</span>' : ''; ?> </h2>

    <?php

    $wrapper_class = '';

    if ( isset( $extra['copy_link'] ) && $extra['copy_link'] > 1 ):
    ?>
    ...
    <div class="multi_regis_form_fields" id="multi_regis_form_fields-<?php echo $event_id; ?>">
...
//print_r( event_espresso_get_is_active($event_id));
        ...
        <?php
                    ...
                    echo event_espresso_add_question_groups( $question_groups, $events_in_session, $event_id, 1, array('attendee_number' => 1));
                    ...
                    if ( $allow_multiple == "Y" )
                    {
                        ...
                        if ( $extra['additional_attendee_reg_info'] > 1 && $extra['additional_attendee'] > 0)
                        {

                            //The offset of 2 since this is attendee 2 and on
                            //Used for "Attendee #" display
                            for ( $i = 2, $cnt = $extra['additional_attendee'] + 2; $i < $cnt; $i++ ) {

                                $extra['attendee_number'] = $i;
                                echo "<h2>" . __('Attendee ', 'event_espresso') . $i . "</h2>";
                                event_espresso_add_question_groups( $question_groups, $events_in_session, $event_id, 1, $extra );
                            }
