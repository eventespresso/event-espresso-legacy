...
?>
<div id="event_espresso_registration_form" class="<?php echo apply_filters('espresso_filter_hook_css_event_display_boxes','event-display-boxes multi-reg-page ui-widget'); ?>">
	...
	<h3 class="<?php echo apply_filters('espresso_filter_hook_css_event_title','event_title ui-widget-header ui-corner-top'); ?>" id="event_title-<?php echo $event_id; ?>">
		<?php echo stripslashes_deep($event_name) ?> <?php echo $is_active['status'] == 'EXPIRED' ? ' - <span class="'.apply_filters('espresso_filter_hook_css_expired_event','expired_event').'">Event Expired</span>' : ''; ?>
	</h3>
	<div class="<?php echo apply_filters('espresso_filter_hook_css_event_data_display','multi_regis_form_fields event-data-display ui-widget-content ui-corner-bottom'); ?>" id="multi_regis_form_fields-<?php echo $event_id . '-' . $meta['price_id']; ?>">
		...
		if ( $display_desc == "Y" && $display_description_on_multi_reg_page != 'N' ) {
			...
			<div class="<?php echo apply_filters('espresso_filter_hook_css_event_description','event_description'); ?>">
			<?php
		...
		switch ($is_active['status']) {
			case 'EXPIRED': //only show the event description.
				_e('<h3 class="'.apply_filters('espresso_filter_hook_css_expired_event','expired_event').'">This event has passed.</h3>', 'event_espresso');
				break;

			case 'REGISTRATION_CLOSED': //only show the event description.
				// if todays date is after $reg_end_date
				?>
				<p class="<?php echo apply_filters('espresso_filter_hook_css_event_full','event_full'); ?>"><strong><?php _e('We are sorry but registration for this event is now closed.', 'event_espresso'); ?></strong></p>
				<p class="<?php echo apply_filters('espresso_filter_hook_css_event_full','event_full'); ?>"><strong><?php _e('Please <a href="contact" title="contact us">contact us</a> if you would like to know if spaces are still available.', 'event_espresso'); ?></strong></p>
				<?php
				break;
			...
			case 'REGISTRATION_NOT_OPEN': //only show the event description.
				...
				?>
				<p class="<?php echo apply_filters('espresso_filter_hook_css_event_full','event_full'); ?>"><strong><?php _e('We are sorry but this event is not yet open for registration.', 'event_espresso'); ?></strong></p>
				<p class="<?php echo apply_filters('espresso_filter_hook_css_event_full','event_full'); ?>"><strong><?php _e('You will be able to register starting ' . event_espresso_no_format_date($reg_start_date, 'F d, Y'), 'event_espresso'); ?></strong></p>
				<?php
				break;
			...
			default:
				...
				?>
				<div class="multi_regis_wrapper_attendee-<?php echo $is_primary; ?> <?php echo apply_filters('espresso_filter_hook_css_multi_regis_wrapper_attendee','multi_regis_wrapper_attendee'); ?>">
					<div class="<?php echo apply_filters('espresso_filter_hook_css_event_display_boxes','event-display-boxes'); ?>">
						<?php
						echo '<h4 class="'.apply_filters('espresso_filter_hook_css_section_heading','section-heading').'"><strong>'.__('Price Type:') . '</strong> ' . stripslashes_deep($meta['price_type']).$attendee_quantity.'</h4>';
						echo '<h3 class="'.apply_filters('espresso_filter_hook_css_section_heading','section-heading').'">' . __('Attendee ', 'event_espresso') . $attendee_number . '</h3>';
						...
						if ( $event_counter == 1 && $event_count > 1 || ($meta['attendee_quantity'] > 1 && $event_meta['additional_attendee_reg_info'] > 1) ) {
							...
							<div class="<?php echo apply_filters('espresso_filter_hook_css_event_messages','event-messages ui-state-highlight'); ?>">
								<p class="<?php echo apply_filters('espresso_filter_hook_css_instruct','instruct'); ?>" style="position:relative;padding:1em;">
									<span class="<?php echo apply_filters('espresso_filter_hook_css_copy_all_button_wrapper','copy-all-button-wrapper'); ?>" style="position:relative;z-index:10;">									
										<?php _e('Copy above information to all forms?', 'event_espresso'); ?> <button type="button" class="<?php echo apply_filters('espresso_filter_hook_css_copy_all_button','copy-all-button'); ?>" value="<?php echo $event_id . '|' . $meta['price_id']; ?>"><?php _e('Yes', 'event_espresso'); ?></button>										
									</span>
									<span class="<?php echo apply_filters('espresso_filter_hook_css_copy_all_button_success','copy-all-button-success'); ?>" style="display:none;position:absolute; top:.2em; left:0;padding:1em; border-radius:3px;z-index:1;background:#DCF3D9;"></span>
								</p>
							</div>
							<?php
						}
					...
					//
						...
						if ($event_meta['additional_attendee_reg_info'] == 1) {
							...
							for ($i = $attendee_number, $cnt = $meta['attendee_quantity'] + $attendee_number - 1; $i < $cnt; $i++) {
								...
								$meta['attendee_number'] = $price_group_att_counter;
								?>
								<hr class="<?php echo apply_filters('espresso_filter_hook_css_hr_additional_attendee','hr_additional_attendee'); ?>" />
								<div class="<?php echo apply_filters('espresso_filter_hook_css_multi_regis_wrapper_attendee_additional','multi_regis_wrapper_attendee-additional'); ?>">
									<div class="<?php echo apply_filters('espresso_filter_hook_css_event_display_boxes','event-display-boxes'); ?>">
										<?php
										echo '<h3 class="'.apply_filters('espresso_filter_hook_css_section_heading','section-heading').'">' . __('Attendee ', 'event_espresso') . $i . '</h3>';
										echo event_espresso_copy_dd($event_id, $meta);
										echo event_espresso_add_question_groups($question_groups, $events_in_session[$event_id], $event_id, 1, $meta);
										?>
									</div>
