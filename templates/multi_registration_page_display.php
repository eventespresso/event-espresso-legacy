...
?>
<div id="event_espresso_registration_form" class="event-display-boxes multi-reg-page <?php echo apply_filters('espresso_filter_hook_registration_css_event-display-boxes','ui-widget'); ?>">
	...
	<h3 class="event_title <?php echo apply_filters('espresso_filter_hook_registration_css_event_title',' ui-widget-header ui-corner-top'); ?>" id="event_title-<?php echo $event_id; ?>">
		<?php echo stripslashes_deep($event_name) ?> <?php echo $is_active['status'] == 'EXPIRED' ? ' - <span class="expired_event '.apply_filters('espresso_filter_hook_registration_css_expired_event','').'">Event Expired</span>' : ''; ?>
	</h3>
	<div class="multi_regis_form_fields event-data-display <?php echo apply_filters('espresso_filter_hook_registration_css_form_wrapper_event-data-display','ui-widget-content ui-corner-bottom'); ?>" id="multi_regis_form_fields-<?php echo $event_id . '-' . $meta['price_id']; ?>">
		...
		if ( $display_desc == "Y" && $display_description_on_multi_reg_page != 'N' ) {
			...
			<div class="event_description <?php echo apply_filters('espresso_filter_hook_registration_css_event_description',''); ?>">
			<?php
		...
		switch ($is_active['status']) {
			...
			case 'REGISTRATION_CLOSED': //only show the event description.
				...
				?>
				<p class="event_full <?php echo apply_filters('espresso_filter_hook_registration_css_event_full',''); ?>"><strong><?php _e('We are sorry but registration for this event is now closed.', 'event_espresso'); ?></strong></p>
				<p class="event_full <?php echo apply_filters('espresso_filter_hook_registration_css_event_full',''); ?>"><strong><?php _e('Please <a href="contact" title="contact us">contact us</a> if you would like to know if spaces are still available.', 'event_espresso'); ?></strong></p>
				<?php
				break;
			...
			case 'REGISTRATION_NOT_OPEN': //only show the event description.
				...
				?>
				<p class="event_full <?php echo apply_filters('espresso_filter_hook_registration_css_event_full',''); ?>"><strong><?php _e('We are sorry but this event is not yet open for registration.', 'event_espresso'); ?></strong></p>
				<p class="event_full <?php echo apply_filters('espresso_filter_hook_registration_css_event_full',''); ?>"><strong><?php _e('You will be able to register starting ' . event_espresso_no_format_date($reg_start_date, 'F d, Y'), 'event_espresso'); ?></strong></p>
				<?php
				break;
			...
			default:
				...
				?>
				<div class="multi_regis_wrapper_attendee-<?php echo $is_primary; ?> <?php echo apply_filters('espresso_filter_hook_registration_css_multi_regis_wrapper_attendee',''); ?>">
					<div class="event-display-boxes <?php echo apply_filters('espresso_filter_hook_registration_css_event-display-boxes',''); ?>">
						<?php
						echo '<h4 class="section-heading '.apply_filters('espresso_filter_hook_registration_css_section-heading','').'"><strong>'.__('Price Type:') . '</strong> ' . stripslashes_deep($meta['price_type']).$attendee_quantity.'</h4>';
						echo '<h3 class="section-heading '.apply_filters('espresso_filter_hook_registration_css_section-heading','').'">' . __('Attendee ', 'event_espresso') . $attendee_number . '</h3>';
						...
						if ( $event_counter == 1 && $event_count > 1 || ($meta['attendee_quantity'] > 1 && $event_meta['additional_attendee_reg_info'] > 1) ) {
							...
							<div class="event-messages <?php echo apply_filters('espresso_filter_hook_registration_css_event-messages','ui-state-highlight'); ?>">
								<p class="instruct <?php echo apply_filters('espresso_filter_hook_registration_css_instruct',''); ?>" style="position:relative;padding:1em;">
									<span class="copy-all-button-wrapper <?php echo apply_filters('espresso_filter_hook_registration_css_copy-all-button-wrapper',''); ?>" style="position:relative;z-index:10;">									
										<?php _e('Copy above information to all forms?', 'event_espresso'); ?> <button type="button" class="copy-all-button <?php echo apply_filters('espresso_filter_hook_registration_css_copy-all-button',''); ?>" value="<?php echo $event_id . '|' . $meta['price_id']; ?>"><?php _e('Yes', 'event_espresso'); ?></button>										
									</span>
									<span class="copy-all-button-success <?php echo apply_filters('espresso_filter_hook_registration_css_copy-all-button-success',''); ?>" style="display:none;position:absolute; top:.2em; left:0;padding:1em; border-radius:3px;z-index:1;background:#DCF3D9;"></span>
								</p>
							</div>
							<?php
						}
					...
					//
						...
						if ($event_meta['additional_attendee_reg_info'] == 1) {
							...
							for ($i = $attendee_number, $cnt = $meta['attendee_quantity'] + $attendee_number - 1; $i < $cnt; $i++) {
								...
								$meta['attendee_number'] = $price_group_att_counter;
								?>
								<hr class="hr_additional_attendee <?php echo apply_filters('espresso_filter_hook_registration_css_hr_additional_attendee',''); ?>" />
								<div class="multi_regis_wrapper_attendee-additional <?php echo apply_filters('espresso_filter_hook_registration_css_multi_regis_wrapper_attendee-additional',''); ?>">
									<div class="event-display-boxes">
										<?php
										echo '<h3 class="section-heading '.apply_filters('espresso_filter_hook_registration_css_multi_regis_wrapper_attendee-additional','').'">' . __('Attendee ', 'event_espresso') . $i . '</h3>';
										echo event_espresso_copy_dd($event_id, $meta);
										echo event_espresso_add_question_groups($question_groups, $events_in_session[$event_id], $event_id, 1, $meta);
										?>
									</div>
