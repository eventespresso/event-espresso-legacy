...
if (!function_exists('register_attendees')) {
	...
	if ($wpdb->get_results($sql)){
			...
			foreach ($events as $event){ 
					...
					echo event_date_display($today, "F j, Y H:i A") . ' ' . $timezone_string . " vs. " . event_date_display(date("Y-m-d")) .' ' .date('H:i A') . ' ' .get_option('timezone_string');
			...
			<div  style="outline: 1px solid rgb(255, 0, 0); padding:10px;"><p><strong><?php _e('Event Timezone', 'event_espresso')?></strong></p>
        <?php echo eventespresso_ddtimezone($event_id);?></div><?php
