        ...
        if ($num_rows > 0) {
           ...
           $location = (!empty($event_address) ? $event_address : '') . (!empty($event_address2) ? '<br />' . $event_address2 : '') . (!empty($event_city) ? '<br />' . $event_city : '') . (!empty($event_state)  ? ', ' . $event_state : '') . (!empty($event_zip) ? '<br />' . $event_zip : '') . (!empty($event_country) ? '<br />' . $event_country : '');
            ...
            if ($num_attendees >= $reg_limit) {
                ?>
                <div class="<?php echo apply_filters('espresso_filter_hook_css_event_full_event_display_boxes','espresso_event_full event-display-boxes'); ?>" id="espresso_event_full-<?php echo $event_id; ?>">
                    <h3 class="<?php echo apply_filters('espresso_filter_hook_css_event_title','event_title'); ?>" id="espresso_event_full-<?php echo $event_id; ?>"><?php echo stripslashes_deep($event_name) ?></h3>
                    <div class="<?php echo apply_filters('espresso_filter_hook_css_event_messages','event-messages ui-state-highlight'); ?>">
                        <p class="<?php echo apply_filters('espresso_filter_hook_css_event_full','event_full'); ?>"><strong><?php _e('We are sorry but this event has reached the maximum number of attendees!', 'event_espresso'); ?></strong></p>
                        <p class="<?php echo apply_filters('espresso_filter_hook_css_event_full','event_full'); ?>"><strong><?php _e('Please check back in the event someone cancels.', 'event_espresso'); ?></strong></p>
                        <p class="<?php echo apply_filters('espresso_filter_hook_css_num_attendees','num_attendees'); ?>"><?php _e('Current Number of Attendees:', 'event_espresso'); ?> <?php echo $num_attendees ?></p>
                    </div>
                <?php
                ...
                if (($num_attendees >= $reg_limit) && ($allow_overflow == 'Y' && $overflow_event_id != 0)) {
                    ?>
                        <p id="register_link-<?php echo $overflow_event_id ?>" class="<?php echo apply_filters('espresso_filter_hook_css_register_link_footer','register-link-footer'); ?>"><a class="<?php echo apply_filters('espresso_filter_hook_css_a_register_link','a_register_link ui-button ui-button-big ui-priority-primary ui-state-default ui-state-hover ui-state-focus ui-corner-all'); ?>" id="a_register_link-<?php echo $overflow_event_id ?>" href="<?php echo espresso_reg_url($overflow_event_id); ?>" title="<?php echo stripslashes_deep($event_name) ?>"><?php _e('Join Waiting List', 'event_espresso'); ?></a></p>
                    <?php } ?>
                </div>
                 ...
                 echo '<h3 class="'.apply_filters('espresso_filter_hook_css_expired_event','expired_event').'">'.__('This event has expired or is no longer available.', 'event_espresso').'</h3>';
            }
