...
<form id="form1" name="form1" method="post" action="<?php echo home_url()?>/?page_id=<?php echo $event_page_id?>">
            ...
            <table width="95%" border="0" id="event_espresso_attendee_verify">
              <tr>
               ...
               <tr>
                <td><strong class="event_espresso_name">
                  <?php echo $price_type == '' ? __('Price per attendee:','event_espresso') : __('Type/Price per attendee:','event_espresso'); ?>
                  </strong></td>
                <td><span class="event_espresso_value"><?php echo $price_type == '' ? $org_options['currency_symbol'] . $event_price : stripslashes_deep($price_type) . ' / ' .$org_options['currency_symbol'].$event_price;?></span></td>
              </tr>
              <tr>
                <td><strong class="event_espresso_name">
				...
				if ( $wpdb->num_rows > 0 ){
					foreach ($x_attendees as $x_attendee){
						//print_r($x_attendees);
						echo "<br/>" . $x_attendee['fname'] . " " . $x_attendee['lname']  . "(" . $x_attendee['email']  . ") ";
