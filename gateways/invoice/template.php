...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Cell(80);
		//Title
		$this->MultiCell(100,10,iconv("UTF-8", "ISO-8859-1",$invoice_payment_settings['pdf_title']),0,'R');//Set the right header
		//Line break
		$this->Ln(20);
	}
...
$pdf->SetTitle(iconv("UTF-8", "ISO-8859-1",$event_name . ' - ' . $invoice_payment_settings['pdf_title']));
...
//Event Data
$data = $pdf->LoadData (array(iconv("UTF-8", "ISO-8859-1",$event_name) . ';' . date('m-d-Y',strtotime($start_date)) . ';' . html_entity_decode($org_options['currency_format'].$amount_pd)));
