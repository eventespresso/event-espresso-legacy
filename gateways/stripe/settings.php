			...
			<div class="inside">
				<div class="padding">
					...
					if (!empty($_REQUEST['activate_stripe'])) {
						$active_gateways['stripe'] = __DIR__;
					...
					if (array_key_exists('stripe', $active_gateways)) {
						...
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_stripe=true\';" class="green_alert pointer"><strong>' . __('Activate Stripe?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
