...
//amount needs to be in cents
	...
	if (isset($_POST['bank_id']) and !empty($_POST['bank_id'])) {
		$return_url = home_url() . '/?page_id=' . $org_options['return_url'] . '&id=' . $payment_data['attendee_id'] . '&registration_id=' . $payment_data['registration_id'];
		$report_url = home_url() . '/?page_id=' . $org_options['notify_url'] . '&id=' . $payment_data['attendee_id'] . '&registration_id=' . $payment_data['registration_id'] . '&event_id=' . $payment_data['event_id'] . '&attendee_action=post_payment&form_action=payment&ideal=1';
