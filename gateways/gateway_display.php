...
function espresso_display_finalize_payment_header($registration_id, $attendee_id) {
	global $org_options;
	?>
	...
	<?php _e('If using one of the offline payment options, please make note of the information below, then', 'event_espresso'); ?>
			<a href="<?php echo home_url() . '/?page_id=' . $org_options['return_url']; ?>&amp;payment_type=cash_check&amp;registration_id=<?php echo $registration_id ?>&amp;id=<?php echo $attendee_id ?>" class="inline-link" title="<?php _e('Finalize your registration', 'event_espresso'); ?>"><?php _e('click here to finalize your registration', 'event_espresso'); ?></a>
		</p>
	</div>
	<?php
}
...
do_action('action_hook_espresso_display_finalize_payment_header', $registration_id, $attendee_id);
