<?php
function event_espresso_paypal_pro_settings(){
	if (isset($_POST['update_paypal_pro'])) {
		//$paypal_pro_settings = get_option('event_espresso_paypal_settings');
			...
			$paypal_pro_settings['paypal_api_credit_cards'] = implode(",",$_POST['paypal_api_credit_cards']);
...
<div class="postbox">
  ...
  <div class="padding">
  <?php
				if ($_REQUEST['activate_paypal_pro'] == 'true'){
				...
				if ($_REQUEST['reactivate_paypal_pro'] == 'true'){
					update_option( 'events_paypal_pro_active', 'true');
					//update_option( 'event_espresso_payment_gateway', 'paypal');
				}
				if ($_REQUEST['deactivate_paypal_pro'] == 'true'){
