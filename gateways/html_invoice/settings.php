...
function event_espresso_html_invoice_payment_settings() {
	global $espresso_premium, $notices, $espresso_wp_user, $org_options, $active_gateways;
	if ($espresso_premium != true)
		return;

	$payment_settings = array();
	$payment_settings['invoice'] = get_option('event_espresso_html_invoice_payment_settings');
	...
	if (isset($_POST['update_invoice_payment_settings'])) {
		...
		if (update_option('event_espresso_html_invoice_payment_settings', $payment_settings['invoice'])) {
	...
	if (empty($payment_settings['invoice'])) {
		...
		if (add_option('event_espresso_html_invoice_payment_settings', $payment_settings['invoice'], '', 'no') == false) {
			update_option('event_espresso_html_invoice_payment_settings', $payment_settings['invoice']);
		}
	}

	//Open or close the postbox div
	if (empty($_REQUEST['deactivate_html_invoice_payment'])
					&& (!empty($_REQUEST['activate_html_invoice_payment'])
					|| array_key_exists('html_invoice', $active_gateways))) {
		$postbox_style = '';
	} else {
	...
	<div class="metabox-holder">
		<div class="postbox <?php echo $postbox_style; ?>">
			...
			<div class="inside">
				<div class="padding">
					<?php
					if (!empty($_REQUEST['activate_html_invoice_payment'])) {
						$active_gateways['html_invoice'] = dirname(__FILE__);
						if (update_option('event_espresso_active_gateways', $active_gateways)) {
					...
					if (!empty($_REQUEST['deactivate_html_invoice_payment'])) {
						unset($active_gateways['html_invoice']);
						if (update_option('event_espresso_active_gateways', $active_gateways)) {
					...
					echo '<ul>';
					if (!array_key_exists('html_invoice', $active_gateways)) {
							echo '<li id="activate_html_invoice" style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_html_invoice_payment=true#html_invoice\';" class="green_alert pointer"><strong>' . __('Activate Invoice Payments?', 'event_espresso') . '</strong></li>';
					} else {
							echo '<li id="deactivate_html_invoice" style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&deactivate_html_invoice_payment=true\';" class="red_alert pointer"><strong>' . __('Deactivate Invoice Payments?', 'event_espresso') . '</strong></li>';
							event_espresso_display_html_invoice_payment_settings();
					}
					echo '</ul>';
					?>
				</div>
...
function event_espresso_display_html_invoice_payment_settings() {

	$payment_settings = array();
	$payment_settings['invoice'] = get_option('event_espresso_html_invoice_payment_settings');

	$files = espresso_html_invoice_template_files();
	$values = array(
			...
			<tbody>
				<tr>
					...
					<td><select id="base-invoice-select" class="chzn-select wide" name="invoice_css">
							<option <?php espresso_html_invoice_is_selected('', $payment_settings['invoice']['invoice_css']) ?> value="simple.css">
								<?php _e('Default CSS - Simple', 'event_espresso'); ?>
							</option>
							<?php foreach ($files as $fname) { ?>
								<option <?php espresso_html_invoice_is_selected($fname, $payment_settings['invoice']['invoice_css']) ?> value="<?php echo $fname ?>"><?php echo $fname; ?></option>
							<?php } ?>
						</select><br />
...
add_action('action_hook_espresso_display_gateway_settings','event_espresso_html_invoice_payment_settings');
