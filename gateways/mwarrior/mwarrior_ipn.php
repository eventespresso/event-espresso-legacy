...
// Enable test mode if needed
	...
	if ($mwarrior->validateIpn()) {
		...
		$payment_data['payment_status'] = "IPN Failed";
		?>
		<h2 style="color:#F00;"><?php _e('There was an error processing your transaction!', 'event_espresso'); ?></h2>
	...
	$payment_data['total_cost'] = $payment_data['amount_pd'];
	$payment_data['txn_details'] = serialize($mwarrior->response);
	$payment_data = apply_filters('action_hook_espresso_update_attendee_payment_data_in_db', $payment_data);
	return $payment_data;
}
