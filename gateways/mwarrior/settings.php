			...
			<div class="inside">
				<div class="padding">
					...
					if (!empty($_REQUEST['activate_mwarrior'])) {
						$active_gateways['mwarrior'] = __DIR__;
					...
					if (array_key_exists('mwarrior', $active_gateways)) {
						...
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_mwarrior=true\';" class="green_alert pointer"><strong>' . __('Activate Merchant Warrior IPN?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
