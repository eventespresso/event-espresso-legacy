			...
			<div class="inside">
				<div class="padding">
					...
					if (array_key_exists('firstdata_connect_2', $active_gateways)) {
						...
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_firstdata_connect_2=true\';" class="green_alert pointer"><strong>' . __('Activate First Data Connect 2?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
			...
			<tr>
				...
				<td valign="top">
					<ul>
						...
						<li>
							<label for="timezone">
								<?php _e('Choose a timezone for the transaction? ', 'event_espresso'); ?><?php echo apply_filters('espresso_help', 'timezone') ?>
							</label>
							<?php
							$values = array(
