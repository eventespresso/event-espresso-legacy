			...
			<div class="inside">
				<div class="padding">
					...
					if (!empty($_REQUEST['activate_paypal_pro'])) {
						$active_gateways['paypal_pro'] = __DIR__;
					...
					if (array_key_exists('paypal_pro', $active_gateways)) {
						...
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_paypal_pro=true\';" class="green_alert pointer"><strong>' . __('Activate PayPal PRO?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
