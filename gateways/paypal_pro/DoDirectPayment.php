...
// Setup PayPal object
		...
		if (!empty($PayPalResult)) {
			...
			if (!APICallSuccessful($PayPalResult['ACK'])) {
				...
				if(empty($transaction_details['TRANSACTIONID'])) {
					...
					$transaction_details = $PayPalResult;
				}
			}
		} else {
			?>
	...
	if(!empty($transaction_details['REQUESTDATA']['PAYMENTACTION']) && $transaction_details['REQUESTDATA']['PAYMENTACTION'] === 'Authorization') {
		$DCFields = array(
							'authorizationid' => $transaction_details['TRANSACTIONID'], 				// Required. The authorization identification number of the payment you want to capture. This is the transaction ID returned from DoExpressCheckoutPayment or DoDirectPayment.
							'amt' => $transaction_details['AMT'], 							// Required. Must have two decimal places.  Decimal separator must be a period (.) and optional thousands separator must be a comma (,)
							...
							'currencycode' => $transaction_details['CURRENCYCODE'], 					// Three-character currency code
							...
							'softdescriptor' => $transaction_details['REQUESTDATA']['DESC'], 				// Per transaction description of the payment that is passed to the customer's credit card statement.
						);
		$PayPalRequestData = array(
				'DCFields' => $DCFields
		);
