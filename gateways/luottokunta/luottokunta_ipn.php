...
function espresso_process_luottokunta($payment_data) {
	...
	if(!$success){
		$display_text='2900'!=$_GET['LKSRC'] ? $_GET['LKMSGTXT'] : __('The card number, validity, or card verification code is invalid. This may also appear when double-authorization has occurred.','espresso_g_luottokunta');?>
		
<h2><?php _e('Payment Declined','espresso_g_luottokunta')?></h2>
		...
		<p><strong class="credit_card_failure"><?php _e('Please try again','espresso_g_luottokunta')?></strong></p>
		<p><?php printf(__("Order ID: %s, Primary Error code: %s, Secondary Error code: %s",'espresso_g_luottokunta'), $order_id, $_GET['LKPRC'],$_GET['LKSRC'])?></p>
		<p><?php _e("If this error persists, you may want to contact the site owners and provide them with the above data.",'espresso_g_luottokunta');?></p>
		<?php
	...
	//if the request says it was successful, check the mac calculations (if teh settings indicate we should)
		...
		if( array_key_exists('LKMAC',$_GET) && $locally_calculated_mac_string == $_GET['LKMAC'] ){
			...
			if(!array_key_exists('LKMAC',$_GET)){
				$mac_error_message = __('The site admin seems to have malconfigured their Luottokunta MAC Security Check Settings. They are not sending the \'MAC check\' the Success URL','espresso_g_luottokunta');
			}else{
				$mac_error_message = sprintf(__('The MAC Security code sent from Luottokunta (%s) does not match the one in Event Espresso (%s)','espresso_g_luottokunta'),$_GET['LKMAC'],$locally_calculated_mac_string);
			}
			?>
				<h2><?php _e('Payment Declined','espresso_g_luottokunta')?></h2>
...
Also check that the amount field is not missing from the implementation.",'espresso_g_luottokunta');
}

		
elseif($primary_error_code == '100' && $secondary_error_code == '102'){ $msg = __("
Also check that the currency field is not missing from the implementation.",'espresso_g_luottokunta');
}
...
If necessary, please contact Luottokunta.",'espresso_g_luottokunta');
}
...
Also check that the card number field is not missing from the implementation.",'espresso_g_luottokunta');
}

		
elseif($primary_error_code == '100' && $secondary_error_code == '105'){ $msg = __("
Also check that the card verification code field is not missing from the implementation.",'espresso_g_luottokunta');
}

		
elseif($primary_error_code == '100' && $secondary_error_code == '106'){ $msg = __("
Also check that the card validity field is not missing from the implementation.",'espresso_g_luottokunta');
}
...
Please check that the order description does not exceed its maximum length. The maximum length of order description field is 3999 characters.",'espresso_g_luottokunta');
}
...
Also check that the Authentication_Mac field is not missing from the implementation.",'espresso_g_luottokunta');
}
...
Also check that the merchant id field is not missing from the implementation.",'espresso_g_luottokunta');
}
...
Please check that the language field contains one of the following values: EN, FI or SE.",'espresso_g_luottokunta');
}

		
elseif($primary_error_code == '100' && $secondary_error_code == '159'){ $msg = __("
Also check that the Card_Details_Transmit field is not missing from the implementation.",'espresso_g_luottokunta');
}


elseif($primary_error_code == '100' && $secondary_error_code == '160'){ $msg = __("
Also check that the Device_Category field is not missing from the implementation.",'espresso_g_luottokunta');
}

		
elseif($primary_error_code == '100' && $secondary_error_code == '161'){ $msg = __("
The order id distinguishes a transaction, and therefore each order id can be used only once.",'espresso_g_luottokunta');
}

		
elseif($primary_error_code == '100' && $secondary_error_code == '162'){ $msg = __("
Also check that the Success_Url field is not missing from the implementation.",'espresso_g_luottokunta');
}


elseif($primary_error_code == '100' && $secondary_error_code == '163'){ $msg = __("
Also check that the Failure_Url field is not missing from the implementation.",'espresso_g_luottokunta');
}

		
elseif($primary_error_code == '100' && $secondary_error_code == '164'){ $msg = __("
Also check that the Transaction_Type field is not missing from the implementation.",'espresso_g_luottokunta');
}


elseif($primary_error_code == '100' && $secondary_error_code == '165'){ $msg = __("
Also check that the Cancel_Url field is not missing from the implementation.",'espresso_g_luottokunta');
}
...
Please check that the customer id contains only numbers and/or letters (a–z and/or A–Z) and is no longer than 64 characters.",'espresso_g_luottokunta');
}
...
Please check that the dossier id contains only numbers and/or letters (a–z and/or A–Z) and is no longer than 64 characters.",'espresso_g_luottokunta');
}
...
Please check that the IP address is accordant with IP protocol version 4 (IPv4). IP address is a 32-bit number, written as four eight-bit numbers, separated by dots.",'espresso_g_luottokunta');
}
...
Please contact Luottokunta and/or American Express or Diners Club if you wish to accept those cards in the distance selling.",'espresso_g_luottokunta');
}
...
Please check that the XML request is made according to the XML schema.",'espresso_g_luottokunta');
}
...
web interface or through the XML interface.",'espresso_g_luottokunta');
}
...
Please check that you are not trying to debit or credit the transaction that has not been authorised yet, and therefore does not exist in Luottokunta ePayment Service.",'espresso_g_luottokunta');
}
...
Maximum amount that can be authorised in Luottokunta ePayment Service at once is 50 000 Euros.",'espresso_g_luottokunta');
}
...
Please check that you are not e.g. trying to credit an authorised transaction or a transaction that is still being processed.",'espresso_g_luottokunta');
}
...
Debit and credit transactions cannot be deleted until the batch is closed.",'espresso_g_luottokunta');
}
...
An authorisation expires after 25 days from the initial authorisation date, after which it cannot be debited anymore.",'espresso_g_luottokunta');
}
...
Please check that the amount is not too big for the aimed function. E.g. the amount to be debited cannot be more than 15% bigger than the initially authorised amount.",'espresso_g_luottokunta');
}
...
If you wish to use another interface, please contact Luottokunta.",'espresso_g_luottokunta');
}
...
Please contact Luottokunta.",'espresso_g_luottokunta');
}
...
If you wish to use another payment channel, please contact Luottokunta.",'espresso_g_luottokunta');
}
...
Also check that you have entered the correct XML password in the XML interface's field <merchantPassword>.",'espresso_g_luottokunta');
}
...
These error codes are only informative information and do not require any actions from the merchant.",'espresso_g_luottokunta');
}
...
You can advice the cardholder to contact his/her card issuer for more detailed information.",'espresso_g_luottokunta');
}
...
The time that authorisation reversal requires varies and is dependent on the card issuer. In case the payment card still has a money reservation, you can advice the cardholder to contact his/her card issuer for more detailed information.",'espresso_g_luottokunta');
}
...
Observe payments for new attempts. If necessary, please contact Luottokunta.",'espresso_g_luottokunta');
}
...
You can advice the cardholder to retry the payment after a while. It is possible that retry will not be successful.",'espresso_g_luottokunta');
}
...
You can advice the cardholder to retry the payment after a while. It is possible that retry will not be successful.",'espresso_g_luottokunta');
	...
	$msg = sprintf(__("No detailed error message for primary error code %s and secondary error code %d",'espresso_g_luottokunta'),$primary_error_code,$secondary_error_code);
}
return $msg;
}