...
function espresso_display_quickpay($payment_data) {
	...
	$transact_url = home_url() . '/?page_id=' . $org_options['return_url'] . '&registration_id=' . $registration_id . '&attendee_action=post_payment&form_action=payment';
	...
	$transact_url = home_url() . '/?page_id=' . $org_options['notify_url'] . '&id=' . $attendee_id . '&registration_id=' . $registration_id . '&attendee_action=post_payment&form_action=payment';
	$params = array('chronopay_callback' => 'true', 'transaction_id' => $transaction_id, 'sessionid' => $sessionid);
	...
	?>
<li><form id="quickpay_form" name="quickpay_form" action="<?php echo $payurl; ?>" method="post">
