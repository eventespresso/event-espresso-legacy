			...
			<div class="inside">
				<div class="padding">
					...
					if (!empty($_REQUEST['activate_quickpay'])) {
						$active_gateways['quickpay'] = __DIR__;
					...
					if (array_key_exists('quickpay', $active_gateways)) {
						...
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_quickpay=true\';" class="green_alert pointer"><strong>' . __('Activate quickpay IPN?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
