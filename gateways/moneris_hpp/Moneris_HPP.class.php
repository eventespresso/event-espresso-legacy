...
*/
class EE_Moneris_HPP extends PaymentGateway {
	...
	public function validateIpn() {
		...
		$post_args = aray(
				...
				'body' => array(
					   'ps_store_id' => 'ozh',
					   'hpp_key' => 'happy',
					   'transactionKey' => 'happy'
					)
			);	
		
		if (function_exists('curl_init')) {

			// parse the moneris_hpp URL
			...
			$error = array();
			// Sets the moneris_hpp address for curl
			$error["set_host"] = curl_setopt( $ch, CURLOPT_URL, $this->_verification_url ); 
			...
			$error["set_post_fields"] = curl_setopt($ch, CURLOPT_POSTFIELDS, $req); 
			
			$errors .= $error["set_host"] ? "Success" : "Failure";
			$errors .= " Setting host: " . $this->_verification_url . "\n";
			...
			// open the connection to moneris_hpp
			...
			if (!$fp) {
				...
				// Post the data back to moneris_hpp
