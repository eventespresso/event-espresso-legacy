...
function event_espresso_firstdata_payment_settings() {
	...
	if (empty($firstdata_settings)) {
		...
		$firstdata_settings['header'] = 'Payment Transactions by FirstData';
	...
	<form method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>">
		<table width="99%" border="0" cellspacing="5" cellpadding="5">
			<tr>
				<td valign="top"><ul>
						...
						<li>
							<label for="use_verify_peer">
								<?php _e('Use the verify peer option in curl', 'event_espresso'); ?>?
							</label>
							<input name="use_verify_peer" type="checkbox" value="1" <?php echo $firstdata_settings['use_verify_peer'] ? 'checked="checked"' : '' ?> />
							&nbsp;<a class="thickbox" href="#TB_inline?height=300&width=400&inlineId=use_verify_peer"><img src="<?php echo EVENT_ESPRESSO_PLUGINFULLURL ?>/images/question-frame.png" width="16" height="16" /></a><br />
						</li>
						<li>
				...
				<td  valign="top"><ul>
					<li><span style="color:red"><?php echo __("Place the .pem file in the following folder.  Make sure the .pem file has the same name as your store number.", 'event_espresso') . ":<br /> " . dirname(__FILE__); ?></span></li>
					<li>
