<?php
function event_espresso_paypal_settings(){
...
<?php
	if (file_exists(EVENT_ESPRESSO_GATEWAY_DIR . "/paypal/Paypal.php")){
				if ($_REQUEST['activate_paypal'] == 'true'){
				...
				if ($_REQUEST['reactivate_paypal'] == 'true'){
					update_option( 'events_paypal_active', 'true');
					//update_option( 'event_espresso_payment_gateway', 'paypal');
				}
				if ($_REQUEST['deactivate_paypal'] == 'true'){
    ...
    <tr>
      <td valign="top"><ul>
          ...
          <li>
            ...
            <br />
            <input type="text" name="button_url" size="34" value="<?php echo (($paypal_settings['button_url'] == '') ? EVENT_ESPRESSO_GATEWAY_URL . "paypal/btn_stdCheckout2.gif" : $paypal_settings['button_url'] ) ;?>" />
      ...
      <td valign="top"><ul>
          <li>
            <label for="surcharge">
              <?php _e('% Surcharge (Added to the total): ','event_espresso'); ?>
            </label>
            <input type="text" name="surcharge" size="5" value="<?php echo (!is_numeric($paypal_settings['surcharge']))?'0.00':$paypal_settings['surcharge'];?>" />
            <a class="ev_reg-fancylink" href="#surcharge"><img src="<?php echo EVENT_ESPRESSO_PLUGINFULLURL?>/images/question-frame.png" width="16" height="16" /></a> </li>
          <li>
		...
		echo select_input('no_shipping', $values, $paypal_settings['no_shipping']); ?>
          ...
          <li>
            <label for="use_sandbox">
              <?php _e('Use the debugging feature and the <a href="https://developer.paypal.com/devscr?cmd=_home||https://cms.paypal.com/us/cgi-bin/?&amp;cmd=_render-content&amp;content_ID=developer/howto_testing_sandbox||https://cms.paypal.com/us/cgi-bin/?&amp;cmd=_render-content&amp;content_ID=developer/howto_testing_sandbox_get_started" title="PayPal Sandbox Login||Sandbox Tutorial||Getting Started with PayPal Sandbox" target="_blank">PayPal Sandbox</a>?','event_espresso'); ?>
            </label>
          ...
          <li>
            ...
            <br />
            <?php echo (($paypal_settings['button_url'] == '') ? '<img src="' . EVENT_ESPRESSO_GATEWAY_URL . 'paypal/btn_stdCheckout2.gif" />' : '<img src="' . $paypal_settings['button_url'] . '" />') ;?></li>
        </ul></td>
    </tr>
  </table>
  <p>
...
<div id="sandbox_info" style="display:none">
  <?php _e('<h2>PayPal Sandbox</h2>
      ...
      <hr />
      <p>The PayPal Sandbox is a testing environment that is a duplicate of the live PayPal site, except that no real money changes hands. The Sandbox allows you to test your entire integration before submitting transactions to the live PayPal environment. Create and manage test accounts, and view emails and API credentials for those test accounts.</p>','event_espresso'); ?>
</div>
...
<div id="currency_info" style="display:none">
  <?php _e('<h2>PayPal Currency</h2>
      <p>PayPal uses 3-character ISO-4217 codes for specifying currencies in fields and variables. </p><p>The default currency code is US Dollars (USD). If you want to require or accept payments in other currencies, select the currency you wish to use. The dropdown lists all currencies that PayPal (currently) supports. ','event_espresso'); ?>
</div>
<div id="surcharge" style="display:none">
  <?php _e('<h2>Payment Surcharge</h2>
      <p>Please enter a decimal number indicating a percent surcharge.
      For example, if you enter 3.00, 3% will be added to the final price of the event during the checkout.
      If the event price is initially $100, the price with the surcharge will be $103.<br />
...
<div id="no_shipping" style="display:none">
  <?php _e('<h2>Shipping Address</h2>
      <p>By default, PayPal will display shipping address information on the PayPal payment screen. If you plan on shipping items to a registrant (shirts, invoices, etc) then use this option. Otherwise it should not be used, as it will require a shipping address when someone registers for an event.</p>','event_espresso'); ?>
</div>
<?php
}
