			...
			<div class="inside">
				<div class="padding">
					...
					if (!empty($_REQUEST['activate_paypal'])) {
						$active_gateways['paypal'] = __DIR__;
					...
					if (array_key_exists('paypal', $active_gateways)) {
						...
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_paypal=true\';" class="green_alert pointer"><strong>' . __('Activate PayPal IPN?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
