...
function espresso_display_paypal($payment_data) {
	...
	foreach ($items as $key => $item) {
		...
		$item_num = $key + 1;
		$item_name = remove_accents( $item->price_option . ' ' . __('for', 'event_espresso') . ' ' . preg_replace( "/[^a-zA-Z0-9\s]/", "", $item->event_name ) . ' ' . __('Attendee:', 'event_espresso') . ' ' . $item->fname . ' ' . $item->lname );
