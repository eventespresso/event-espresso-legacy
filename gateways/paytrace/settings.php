			...
			<div class="inside">
				<div class="padding">
					...
					if (!empty($_REQUEST['activate_paytrace'])) {
						$active_gateways['paytrace'] = __DIR__;
					...
					if (array_key_exists('paytrace', $active_gateways)) {
						...
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_paytrace=true\';" class="green_alert pointer"><strong>' . __('Activate Paytrace?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
