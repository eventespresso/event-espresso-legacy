...
function espresso_process_paytrace($payment_data) {
	...
	if ($payment_data['attendee_id'] > 0 && !empty($primary_registration_id) && strlen($primary_registration_id) > 0) {
		...
		$response = $cls_paytrace->do_transaction($amount_pd, $discount, $line_item, $cc, $csc, $exp_month, $exp_year, $csc, $bname, $baddress, $bcity, $bzip, $email);
		if (isset($response['status'])) {
			...
			$payment_data['total_cost'] = $amount_pd;
...
add_filter('filter_hook_espresso_transactions_get_payment_data', 'espresso_process_paytrace');