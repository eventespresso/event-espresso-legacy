<?php 
function event_espresso_authnet_settings(){
...
<div class="postbox">
<h3>
  <?php _e('Authorize.net Settings','event_espresso'); ?>
</h3>
<?php
	if (file_exists(EVENT_ESPRESSO_GATEWAY_DIR . "/authnet/Authorize.php")){
				if ($_REQUEST['activate_authnet'] == 'true'){
				...
				if ($_REQUEST['reactivate_authnet'] == 'true'){
					update_option( 'events_authnet_active', 'true');
					//update_option( 'event_espresso_payment_gateway', 'authnet');
				}
				if ($_REQUEST['deactivate_authnet'] == 'true'){
				...
				switch (get_option('events_authnet_active')){
					case 'false':
						echo '<li>Authorize.php Gateway is Installed.</li>';
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&reactivate_authnet=true\';" class="green_alert pointer"><strong>' . __('Activate Authorize.net?','event_espresso') . '</strong></li>';
					break;
					case 'true':
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&deactivate_authnet=true\';" class="red_alert pointer"><strong>' . __('Deactivate Authorize.net?','event_espresso') . '</strong></li>';
						event_espresso_display_authnet_settings();
					break;
					default:
						echo '<li style="width:50%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_authnet=true\';" class="yellow_alert pointer"><strong>' . __('The Authorize.net gateway is installed. Would you like to activate it?','event_espresso') . '</strong></li>';
					break;
				}
				echo '</ul>';
	}
    ...
    <tr>
      <td valign="top"><ul>
          ...
          <li>
            ...
            <br />
            <input type="text" name="button_url" size="35" value="<?php echo (($authnet_settings['button_url'] == '') ? EVENT_ESPRESSO_GATEWAY_URL . "authnet/btn_cc_vmad.gif" : $authnet_settings['button_url'] ) ;?>" />
          ...
          <li>
            ...
            <?php _e('(used for your business/personal logo on the Authorize.net payment page)','event_espresso'); ?>
          </li>
        </ul></td>
      <td valign="top"><ul>
          <li>
            ...
            <br />
            <span class="display-path" style="background-color: rgb(255, 251, 204); border:#999 solid 1px; padding:2px;"><?php echo get_option('siteurl').'/?page_id='.$org_options['notify_url']; ?></span> &nbsp;<a class="ev_reg-fancylink" href="#relay_response"><img src="<?php echo EVENT_ESPRESSO_PLUGINFULLURL?>/images/question-frame.png" width="16" height="16" /></a> </li>
          <li>
            <label for="use_sandbox">
              <?php _e('Use the test mode feature for Autorize.net?','event_espresso'); ?>
            </label>
		...
		echo select_input('bypass_payment_page', $values, $authnet_settings['bypass_payment_page']); ?>
          ...
          <li>
            ...
            <br />
            <?php echo (($authnet_settings['button_url'] == '') ? '<img src="' . EVENT_ESPRESSO_GATEWAY_URL . 'authnet/btn_cc_vmad.gif" />' : '<img src="' . $authnet_settings['button_url'] . '" />') ;?></li>
        </ul></td>
    </tr>
  </table>
  <p>
    <input type="hidden" name="update_authnet" value="update_authnet">
    <input class="button-primary" type="submit" name="Submit" value="<?php  _e('Update Authorize.net Settings','event_espresso') ?>" id="save_authnet_settings" />
  </p>
</form>
<div id="relay_response" style="display:none">
  <?php _e('<h2>Relay Response</h2>
      <p>This shows the specific the URL to which the gateway should return the relay response for a transaction. This the page should be set in your Authorize.net account. Login to Authorize.net, goto Account > Response/Receipt URLs > Add URL and enter the following URL.</p>
	  <p><strong>Relay Response URL:</strong> ' . get_option('siteurl').'/?page_id='.$org_options['notify_url'] . '<br />
	  <span style="color:red;">Note:</span> This URL can be changed in the "Organization Settings" page.</p>
	  <p>For complete information on configuring relay response, please refer to <a href="https://account.authorize.net/help/Merchant_Interface_RoboHelp_Project.htm#Miscellaneous/Reference.htm%3E%3Epan=2">Reference &amp; User Guides</a>.</p>','event_espresso'); ?>
</div>
<div id="authnet_image_url_info" style="display:none">
  <h2>
    <?php _e('Authorize.net Image URL (logo for payment page)','event_espresso'); ?>
  </h2>
  <p>
...
<div id="authnet_sandbox" style="display:none">
  <?php _e('<h2>Authorize.net Test Mode</h2>
      <p>Test Mode allows you to submit test transactions to the payment gateway. Transactions that are submitted while Test Mode is ON are NOT actually processed. The result of a transaction depends on the card number submitted, and the invoice amount. If you want a transaction to be approved, use one of the following card numbers.</p><p>370000000000002 (American Express)<br />
6011000000000012 (Discover)<br />
5424000000000015 (Master Card)<br />
4007000000027 (Visa)<br />
</p>','event_espresso'); ?>
</div>
<?php
}
event_espresso_authnet_settings();