...
function event_espresso_authnet_payment_settings() {
	...
	if (empty($authnet_settings)) {
		...
		$authnet_settings['bypass_payment_page'] = '';
			...
			<div class="inside">
				<div class="padding">
					...
					if (!empty($_REQUEST['activate_authnet'])) {
						$active_gateways['authnet'] = __DIR__;
					...
					if (array_key_exists('authnet', $active_gateways)) {
						...
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_authnet=true\';" class="green_alert pointer"><strong>' . __('Activate Authorize.net SIM Gateway?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
				...
				<td valign="top"><ul>
						...
						<li>
							<label for="use_sandbox">
								<?php _e('Is this an account on Authorize.net\'s development server?', 'event_espresso'); ?>
							</label>
