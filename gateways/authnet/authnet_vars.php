...
// Setup class
	...
	$myAuthorize->addField('x_Description', espresso_sanitize_gateway_field( stripslashes_deep( $event_name ) . ' | ' . __('Reg. ID:', 'event_espresso') . ' ' . $attendee_id . ' | ' . __('Name:', 'event_espresso') . ' ' . stripslashes_deep($fname . ' ' . $lname) . ' | ' . __('Total Registrants:', 'event_espresso') . ' ' . $quantity ) );
...
//Post variables
	...
	foreach ($items as $key=>$item) {
		$item_num=$key+1;
		$item_event_name = espresso_sanitize_gateway_field( $item->event_name );
		$item_fname = espresso_sanitize_gateway_field( $item->fname );
		$item_lname = espresso_sanitize_gateway_field( $item->lname );
		$item_price_option = espresso_sanitize_gateway_field ($item->price_option );
