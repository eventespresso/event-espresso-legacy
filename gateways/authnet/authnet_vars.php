...
// Setup class
      ...
      $image_url = $authnet_settings['image_url'];
			...
			$myAuthorize->addField('x_Relay_URL', get_option('siteurl').'/?page_id='.$org_options['notify_url']);
			$myAuthorize->addField('x_Description', $event_name . ' | '.__('Reg. ID:','event_espresso').' '.$attendee_id. ' | '.__('Name:','event_espresso').' '. $fname . ' ' . $lname .' | '.__('Total Registrants:','event_espresso').' '.espresso_count_attendees_for_registration($attendee_id));
			...
			$myAuthorize->addField('x_Zip', $zip);
				...
				if ($authnet_settings['bypass_payment_page'] == 'Y'){
					...
					if ($authnet_settings['button_url']==''){
