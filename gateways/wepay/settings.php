			...
			<div class="inside">
				<div class="padding">
					...
					if (array_key_exists('wepay', $active_gateways)) {
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&deactivate_wepay=true\';" class="red_alert pointer"><strong>' . __('Deactivate Wepay IPN?', 'event_espresso') . '</strong></li>';
						event_espresso_display_wepay_settings($need_to_reauthorize);
					} else {
						echo '<li style="width:30%;" onclick="location.href=\'' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=payment_gateways&activate_wepay=true#wepay\';" class="green_alert pointer"><strong>' . __('Activate Wepay IPN?', 'event_espresso') . '</strong></li>';
					}
					echo '</ul>';
					?>
				</div>
